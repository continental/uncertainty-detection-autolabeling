...
class CalibrateClass:
    ...
    def __init__(
        ...
        self.calib_methods = [
            ...
            "unc_classification_ts_all",
            "unc_classification_ts_percls",
            "unc_classification_iso_all",
            "unc_classification_iso_percls",
        ]
        self._import_calib_class()
        self.calib_method = calib_method
        self.logits = logits
        if uncert is not None:
            self.uncert = uncert[0]  # Since it is contained in a list
        else:
            self.uncert = uncert

        if y_true is not None:
            print(
                "Original Misclassification Error:",
                len(np.where(np.argmax(logits, axis=-1) != y_true)[0]) / len(y_true),
            )
        self.y_true = y_true
    ...
    def _perform_class_calib(self, calib_method):
        ...
        if self.uncert is not None:
            ...
            if self.y_true is not None:
                samp_y_true = np.stack([self.y_true] * 10, axis=0).flatten()
            if self.y_true is not None:
                print(
                    "Post-sampling Misclassification Error:",
                    len(np.where(np.argmax(samp_logits, axis=-1) != samp_y_true)[0])
                    / len(samp_y_true),
                )
        else:
        ...
        if "ts" in calib_method:
            ...
            if "all" in calib_method:
                calib_logits_post = iso.predict(calib_logits.flatten()).reshape(
                    calib_logits.shape
                )
            else:
                calib_logits_post = [
                    iso[i].predict(calib_logits[:, i])
                    for i in range(self.logits.shape[-1])
                ]
                calib_logits_post = np.stack(calib_logits_post, axis=1)
                # necessary to recalibrate into sum 1
            new_probab = calib_logits_post / np.stack(
                [np.sum(calib_logits_post, axis=-1)] * self.logits.shape[-1], axis=-1
            )
            if self.y_true is not None:
                print(
                    "Post-calibration Misclassification Error:",
                    len(np.where(np.argmax(new_probab, axis=-1) != samp_y_true)[0])
                    / len(samp_y_true),
                )
        else:
        ...
        if self.uncert is not None:
            ...
            if self.y_true is not None:
                print(
                    "Post-averaging Misclassification Error:",
                    len(np.where(np.argmax(new_probab, axis=-1) != self.y_true)[0])
                    / len(self.y_true),
                )
            new_entropy = -np.sum(
                new_probab * np.nan_to_num(np.log2(np.maximum(new_probab, 10**-7))),
                axis=1,
            )
    ...
    def calibrate_class(self):
        ...
        if "classification_" + self.available_calib[0] in self.calibrators:
            calib_class_tsall = self._perform_class_calib(self.available_calib[0])
        else:
            calib_class_tsall = [np.array([]), np.array([]), np.array([])]

        if "classification_" + self.available_calib[1] in self.calibrators:
            calib_class_tspcl = self._perform_class_calib(self.available_calib[1])
        else:
            calib_class_tspcl = [np.array([]), np.array([]), np.array([])]

        if "classification_" + self.available_calib[2] in self.calibrators:
            calib_class_isoall = self._perform_class_calib(self.available_calib[2])
        else:
            calib_class_isoall = [np.array([]), np.array([]), np.array([])]

        if "classification_" + self.available_calib[3] in self.calibrators:
            calib_class_isopcl = self._perform_class_calib(self.available_calib[3])
        else:
            calib_class_isopcl = [np.array([]), np.array([]), np.array([])]

        if self.uncert is not None:
            select_uncert_mcclass = np.array([])
        select_entropy = np.array([])
        if (
            self.calib_method == self.available_calib[0]
            and "classification" + self.available_calib[0] in self.calibrators
        ):
            if self.uncert is not None:
                select_uncert_mcclass = calib_class_tsall[2]
            select_entropy = calib_class_tsall[0]
        elif (
            self.calib_method == self.available_calib[1]
            and "classification" + self.available_calib[1] in self.calibrators
        ):
            if self.uncert is not None:
                select_uncert_mcclass = calib_class_tspcl[2]
            select_entropy = calib_class_tspcl[0]
        elif (
            self.calib_method == self.available_calib[2]
            and "classification" + self.available_calib[2] in self.calibrators
        ):
            if self.uncert is not None:
                select_uncert_mcclass = calib_class_isoall[2]
            select_entropy = calib_class_isoall[0]
        elif (
            self.calib_method == self.available_calib[3]
            and "classification" + self.available_calib[3] in self.calibrators
        ):
            if self.uncert is not None:
                select_uncert_mcclass = calib_class_isopcl[2]
            select_entropy = calib_class_isopcl[0]

        if self.uncert is not None:
            return (
                select_uncert_mcclass,
                select_entropy,
                calib_class_tsall[1],
                calib_class_tsall[2],
                calib_class_tsall[0],
                ...
                calib_class_tspcl[2],
                calib_class_tspcl[0],
                calib_class_isoall[1],
                calib_class_isoall[2],
                calib_class_isoall[0],
                calib_class_isopcl[1],
                calib_class_isopcl[2],
                calib_class_isopcl[0],
            )
        else:
            return (
                select_entropy,
...
class Count_class_instances:
    ...
    def _collect_instances(self, labels):
        ...
        for i in range(len(labels)):
            ...
            self.collect_perclass = [
                self.collect_perclass[i] + len(occurances[occurances == i + 1])
    ...
    def _plot_class_dist(self):
        ...
        save_path = os.path.join(
            ...
            "misc",
            f"classes_distribution_{self.config.label_map}.png",
        )
        ...
        train_bar = plt.bar(
            x_axis - 0.2, self.count_train_labels, color="b", width=0.4, label="Train"
        )
        val_bar = plt.bar(
            x_axis + 0.2, self.count_val_labels, color="g", width=0.4, label="Val"
        )
        plt.xticks(x_axis, self.class_names, rotation="vertical")
        plt.ylabel("Count")
        plt.legend(loc="upper right")

        # Calculate maximum bar height for consistent text placement
        max_height = max(max(self.count_train_labels), max(self.count_val_labels))

        # Adding text above each train bar
        for i, p in enumerate(train_bar):
            width = p.get_width()
            x, y = p.get_xy()
            plt.text(
                x + width / 2,
                max_height
                + 0.07 * max_height,  # Fixed offset from the maximum bar height
        ...
        # Adding text above each validation bar
        for i, p in enumerate(val_bar):
            width = p.get_width()
            x, y = p.get_xy()
            plt.text(
                x + width / 2,
    ...
    def count_dataset(self, num_batches, plot_dist=False):
        ...
        if val_count != self.config.eval_samples / self.config.batch_size:
            logging.info(
                "Class instances: Number of validation samples not directly dividable by batch size, slight variation might occur. Difference is "
                + str(
                    abs(val_count - self.config.eval_samples / self.config.batch_size)
        ...
        if train_count != num_batches / self.config.batch_size:
            logging.info(
                "Class instances: Number of training samples not directly dividable by batch size, slight variation might occur. Difference is "
                + str(
                    abs(train_count - num_batches / self.config.batch_size)
                    * self.config.batch_size
                )
                + " images."
            )
        ...
        self.count_mean_train = np.round(
            np.asarray(self.count_train_labels) / sum(self.count_train_labels) * 100, 2
        )
        self.count_mean_val = np.round(
            np.asarray(self.count_val_labels) / sum(self.count_val_labels) * 100, 2
        )
        for i in range(self.config.num_classes):
            logging.info(
                "Amount of objects of class {} in training data is {}, ({}%)".format(
                    self.class_names[i],
                    self.count_train_labels[i],
                    self.count_mean_train[i],
                )
            )
            logging.info(
                "Amount of objects of class {} in validation data is {}, ({}%)".format(
                    self.class_names[i],
                    self.count_val_labels[i],
                    self.count_mean_val[i],
                )
            )

        if plot_dist:
            self._plot_class_dist()
