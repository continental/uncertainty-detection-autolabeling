...
from utils_box import calc_ece, calc_nll, calc_rmse
...
class RegressionCalib:
    ...
    def __init__(
        self, gt_boxes, pred_boxes, box_uncert, gt_classes, model_name, general_path
    ):
    ...
    def _relative_calc_ece(norm_residuals, norm_box_uncert):
        ...
        for i in range(n_intervals):
            interval_fit = np.less_equal(
                norm_residuals,
                np.abs(norm_box_uncert * stats.norm.ppf((1 - p_m[i]) / 2)),
            )
    ...
    def _plot_stdvsdiff(self, residuals, box_uncert, iso_reg, split=0, label="ymin"):
        ...
        """
        res_dict = {
            "x_range": (float(iso_reg.X_min_), float(iso_reg.X_max_)),
        ...
        plt.figure()
        plt.scatter(
            box_uncert[split:].flatten(), residuals[split:].flatten(), alpha=0.3
        )
        plt.plot(
            res_dict["x_thresholds"],
            res_dict["y_thresholds"],
            "-o",
            color="tab:red",
            label="Iso Curve",
        )
        ...
        plt.savefig(
            self.saving_path
            + self.model_name
            + "/regres_plots/regression_calibration_"
            + label
            + "_std.png",
            bbox_inches="tight",
        )
    ...
    def _plot_intervals(
        ...
        for i in range(len(calib_box_uncert)):
            if (
                pred_boxes[split:][i] - select_uncert[i]
                <= gt_boxes[split:][i]
                <= pred_boxes[split:][i] + select_uncert[i]
            ):
                correct_all += 1
        ...
        plt.title(
            "% of true values in interval \u00B1 \u03C3 of predicted values \n out of total detections: "
            + str(round(correct_all, 1))
            + "%"
        )
        if calib:
            plt.plot(
                x,
                cut_pred_boxes + cut_select_uncert,
                label="Pred. upper limit, calib.",
                color="black",
                alpha=0.5,
            )
            plt.plot(
    ...
    def _conf_all(
        ...
        for i in range(len(calib_box_uncert)):
            if (
                pred_boxes[split:][i] - select_uncert[i] <= gt_boxes[split:][i]
            ).all() and (
                gt_boxes[split:][i] <= pred_boxes[split:][i] + select_uncert[i]
            ).all():
                correct_all += 1
        correct_all /= len(calib_box_uncert)
        correct_all *= 100
        ...
        if calib:
            log_percent = (
                "% of true values in interval \u00B1 \u03C3 of predicted values for all coordinates after calibration with "
                + method
                + ": "
                + str(round(correct_all, 1))
            )
            ece = np.round(
                calc_ece(
                    gt_boxes[split:].flatten(),
                    pred_boxes[split:].flatten(),
                    calib_box_uncert.flatten(),
                ),
                4,
            )
            ...
            rmsu = np.round(
                calc_rmse(residuals.flatten().flatten(), calib_box_uncert.flatten()), 4
            )
            log_rmsu = "Calibrated RMSUE " + str(rmsu)
            sharp = np.round(
                np.sqrt(
                    np.mean(np.square(calib_box_uncert.astype(np.float32).flatten()))
                ),
                4,
            )
            ...
            log_percent = (
                "% of true values in interval \u00B1 \u03C3 of predicted values for all coordinates: "
                + str(round(correct_all, 1))
            )
            ece = np.round(
                calc_ece(
            ...
            nll = np.round(
                calc_nll(
                    np.abs(gt_boxes[split:] - pred_boxes[split:]).flatten(),
                    box_uncert[split:].flatten(),
                ),
                4,
            )
            ...
            rmsu = np.round(
                calc_rmse(residuals.flatten().flatten(), box_uncert[split:].flatten()),
                4,
            )
            log_rmsu = "Uncalibrated RMSUE " + str(rmsu)
            sharp = np.round(
                np.sqrt(
                    np.mean(np.square(box_uncert[split:].astype(np.float32).flatten()))
                ),
                4,
            )
        ...
        with open(
            self.saving_path + self.model_name + "/regression_logging.txt", "a"
        ) as f:
            f.write(
                log_percent
                + " \n"
    ...
    def _iso_regression(self, gt_boxes, pred_boxes, box_uncert, split, label):
        ...
        with open(
            ...
            f.write("Isotonic regression: " + label + " \n")
        ...
        if len(gt_boxes.shape) == 1:
            iso_reg = IsotonicRegression(increasing=True, out_of_bounds="clip").fit(
                box_uncert[:split], residuals[:split]
            )
            ...
            self._plot_boxcalib(
                gt_boxes[split:], pred_boxes[split:], box_uncert[split:], "Original"
            )
            self._plot_boxcalib(
                gt_boxes[split:], pred_boxes[split:], calib_box_uncert, "Calibrated"
            )
            ...
            self._plot_intervals(
                gt_boxes, pred_boxes, box_uncert, calib_box_uncert, split, label
            )
            self._plot_intervals(
                gt_boxes, pred_boxes, box_uncert, calib_box_uncert, split, label, True
            )
            ...
            iso_reg = IsotonicRegression(increasing=True, out_of_bounds="clip").fit(
                box_uncert, residuals
            )  # On all data
            self._plot_stdvsdiff(residuals, box_uncert, iso_reg, 0, label)
        else:
            iso_reg = IsotonicRegression(increasing=True, out_of_bounds="clip").fit(
                box_uncert[:split].flatten(), residuals[:split].flatten()
            )
            calib_box_uncert = iso_reg.predict(box_uncert[split:].flatten()).reshape(
                [-1, 4]
            )
            ...
            iso_reg = IsotonicRegression(increasing=True, out_of_bounds="clip").fit(
                box_uncert.flatten(), residuals.flatten()
            )
    ...
    def _temp_scaling(self, gt_boxes, pred_boxes, box_uncert, split, label):
        ...
        with open(
            ...
            f.write("Temperature scaling: " + label + " \n")
        ...
        def f_x():
            ...
            return tf.math.sqrt(
                tf.reduce_mean(
                    tf.math.square(
                        tf.cast(
                            tf.math.abs(gt_boxes[:split] - pred_boxes[:split]),
                            dtype=tf.float32,
                        )
                        - tf.math.divide_no_nan(
                            tf.cast(box_uncert[:split], dtype=tf.float32), tf.abs(x)
                        )
                    )
                )
            )
        ...
        if len(gt_boxes.shape) == 1:
            ...
            self._conf_all(
                gt_boxes, pred_boxes, box_uncert, box_uncert[split:] / temp, split, True
            )
        ...
        def f_y():
            ...
            return tf.reduce_mean(
                tf.math.abs(
                    tf.cast(tf.math.abs(gt_boxes - pred_boxes), dtype=tf.float32)
                    - tf.math.divide_no_nan(
                        tf.cast(box_uncert, dtype=tf.float32), tf.abs(y)
                    )
                )
            )
    ...
    def _rel_temp_scaling(self, norm_box_uncert, norm_residuals, norm):
        ...
        def f_x():
            ...
            return tf.reduce_mean(
                tf.math.abs(
                    norm_residuals - tf.math.divide_no_nan(norm_box_uncert, tf.abs(x))
                )
            )
    ...
    def _absolute_calibration(self, split):
        ...
        with open(
            ...
            f.write("Absolute calibration \n")
        ymin_calib = self._iso_regression(
            self.gt_boxes[:, 0],
            self.pred_boxes[:, 0],
            self.box_uncert[:, 0],
            split,
        ...
        xmin_calib = self._iso_regression(
            self.gt_boxes[:, 1],
            self.pred_boxes[:, 1],
            self.box_uncert[:, 1],
            split,
        ...
        ymax_calib = self._iso_regression(
            self.gt_boxes[:, 2],
            self.pred_boxes[:, 2],
            self.box_uncert[:, 2],
            split,
        ...
        xmax_calib = self._iso_regression(
            self.gt_boxes[:, 3],
            self.pred_boxes[:, 3],
            self.box_uncert[:, 3],
            split,
        ...
        with open(
            self.saving_path
            + self.model_name
            + "/regres_models/regression_calib_iso_pcoo",
            "wb",
        ) as fp:
        ...
        with open(
            self.saving_path
            + self.model_name
            + "/regres_models/regression_calib_ts_pcoo",
            "wb",
        ) as fp:
        ...
        iso_all = self._iso_regression(
            self.gt_boxes, self.pred_boxes, self.box_uncert, split, label="all"
        )
        temp_all = self._temp_scaling(
            self.gt_boxes, self.pred_boxes, self.box_uncert, split, label="all"
        )
        ...
        with open(
            self.saving_path
            + self.model_name
            + "/regres_models/regression_calib_ts_all",
            "wb",
        ) as fp:
            pickle.dump(temp_all, fp)
        with open(
            self.saving_path
            + self.model_name
            + "/regres_models/regression_calib_iso_all",
            "wb",
        ) as fp:
            pickle.dump(iso_all, fp)

        with open(
            self.saving_path + self.model_name + "/regression_logging.txt", "a"
        ) as f:
            f.write(
                "################################################## End absolute calibration ##################################################"
                + " \n\n"
            )
    ...
    def _relative_calibration(self, split):
        ...
        norm_residuals = np.divide(
            np.abs(self.gt_boxes - self.pred_boxes),
            norm,
            out=np.zeros_like(np.abs(self.gt_boxes - self.pred_boxes)),
            where=norm != 0,
            dtype=np.float16,
        )
        norm_box_uncert = np.divide(
            self.box_uncert,
            norm,
            out=np.zeros_like(self.box_uncert),
            where=norm != 0,
            dtype=np.float16,
        )
        ...
        with open(
            ...
            f.write("Relative isotonic regression: all \n")
        norm_iso_reg = IsotonicRegression(increasing=True, out_of_bounds="clip").fit(
            norm_box_uncert[:split].flatten(), norm_residuals[:split].flatten()
        )
        norm_calib_box_uncert = norm_iso_reg.predict(
            norm_box_uncert[split:].flatten()
        ).reshape([-1, 4])
        self._conf_all(
            self.gt_boxes[split:],
            self.pred_boxes[split:],
            norm_box_uncert[split:] * norm[split:],
            norm_calib_box_uncert * norm[split:],
            0,
        )
        self._conf_all(
            self.gt_boxes[split:],
            ...
            0,
            True,
            method="relative IS ALL",
        )

        norm_iso_reg = IsotonicRegression(increasing=True, out_of_bounds="clip").fit(
            norm_box_uncert.flatten(), norm_residuals.flatten()
        )
        with open(
            self.saving_path
            + self.model_name
            + "/regres_models/regression_calib_iso_all_relative",
            "wb",
        ) as fp:
        ...
        with open(
            self.saving_path
            + self.model_name
            + "/regres_models/regression_calib_iso_pcoo_relative",
            "wb",
        ) as fp:
            for i in range(4):
                with open(
                    ...
                    f.write("Relative isotonic regression: " + labels[i] + " \n")
                norm_iso_reg = IsotonicRegression(
                    increasing=True, out_of_bounds="clip"
                ).fit(norm_box_uncert[:split][:, i], norm_residuals[:split][:, i])
                norm_calib_box_uncert = norm_iso_reg.predict(
                    norm_box_uncert[split:][:, i]
                )
                self._conf_all(
                    self.gt_boxes[split:][:, i],
                    self.pred_boxes[split:][:, i],
                    norm_box_uncert[split:][:, i] * norm[split:][:, i],
                    norm_calib_box_uncert * norm[split:][:, i],
                    0,
                )
                self._conf_all(
                    self.gt_boxes[split:][:, i],
                    ...
                    0,
                    True,
                    method="relative IS-" + labels[i],
                )
                norm_iso_reg = IsotonicRegression(
                    increasing=True, out_of_bounds="clip"
                ).fit(self.box_uncert[:, i], norm_residuals[:, i])
                pickle.dump(norm_iso_reg, fp)
        ...
        with open(
            ...
            f.write("Relative temperature scaling: all \n")
        norm_temp = self._rel_temp_scaling(
            norm_box_uncert[:split], norm_residuals[:split], norm[:split]
        )
        with open(
            self.saving_path + self.model_name + "/regression_logging.txt", "a"
        ) as f:
            f.write("Temperature: " + str(norm_temp) + " \n")
        self._conf_all(
            self.gt_boxes[split:],
            self.pred_boxes[split:],
            norm_box_uncert[split:] * norm[split:],
            norm_box_uncert[split:] / norm_temp * norm[split:],
            0,
            True,
            method="relative TS All",
        )
        ...
        with open(
            self.saving_path
            + self.model_name
            + "/regres_models/regression_calib_ts_all_relative",
            "wb",
        ) as fp:
        ...
        with open(
            self.saving_path
            + self.model_name
            + "/regres_models/regression_calib_ts_pcoo_relative",
            "wb",
        ) as fp:
            for i in range(4):
                with open(
                    ...
                    f.write("Relative temperature scaling: " + labels[i] + " \n")
                norm_temp = self._rel_temp_scaling(
                    norm_box_uncert[:split][:, i],
                    norm_residuals[:split][:, i],
                    norm[:split][:, i],
                )
                with open(
                    self.saving_path + self.model_name + "/regression_logging.txt", "a"
                ...
                self._conf_all(
                    ...
                    self.pred_boxes[split:][:, i],
                    norm_box_uncert[split:][:, i] * norm[split:][:, i],
                    norm_box_uncert[split:][:, i] / norm_temp * norm[split:][:, i],
                    0,
                    True,
                    method="Relative TS-" + labels[i],
                )
                norm_temp = self._rel_temp_scaling(
                    norm_box_uncert[:, i], norm_residuals[:, i], norm[:, i]
                )
                pickle.dump(norm_temp, fp)
    ...
    def _per_class_calibration(self, split):
        ...
        with open(
            ...
            f.write("Per-class calibration \n")

        # Calibrate per-class per-coordinate
        if np.min(self.gt_classes) == 0:
            num_classes = int(np.max(self.gt_classes)) + 1
        else:
        ...
        for i in range(num_classes):
            for j in range(4):
                residuals = np.abs(self.gt_boxes[:, j] - self.pred_boxes[:, j])[:split][
                    self.gt_classes[:split] == i
                ]
                if len(residuals) == 0:
                    print(
                        "Stopped per class calibration due some classe/s not having any detections, change calibration method in hparams_config.py accordingly."
                    )
                    sys.exit()
                calibrators.append(
                    IsotonicRegression(increasing=True, out_of_bounds="clip").fit(
                        self.box_uncert[:, j][:split][
                            self.gt_classes[:split] == i
                        ].flatten(),
                        residuals.flatten(),
                    )
        ...
        for i in range(num_classes):
            for j in range(4):
                calib_box_uncert[:, j][self.gt_classes[split:] == i] = calibrators[
                    i, j
                ].predict(self.box_uncert[:, j][split:][self.gt_classes[split:] == i])
                self._conf_all(
                    self.gt_boxes[:, j][split:][self.gt_classes[split:] == i],
                    self.pred_boxes[:, j][split:][self.gt_classes[split:] == i],
                    self.box_uncert[:, j][split:][self.gt_classes[split:] == i],
                    calib_box_uncert[:, j][self.gt_classes[split:] == i],
                    0,
                )
                self._conf_all(
                    self.gt_boxes[:, j][split:][self.gt_classes[split:] == i],
                    ...
                    0,
                    True,
                    method="per class IS-" + lbls[j] + " for class " + str(i),
                )
        ...
        for i in range(num_classes):
            for j in range(4):
                residuals = np.abs(self.gt_boxes[:, j] - self.pred_boxes[:, j])[
                    self.gt_classes == i
                ]
                calibrators.append(
                    IsotonicRegression(increasing=True, out_of_bounds="clip").fit(
                        self.box_uncert[:, j][self.gt_classes == i].flatten(),
                        residuals.flatten(),
                    )
                )
        ...
        with open(
            self.saving_path
            + self.model_name
            + "/regres_models/regression_calib_iso_perclscoo",
            "wb",
        ) as fp:
        ...
        for i in range(num_classes):
            for j in range(4):
                calibrators.append(
                    IsotonicRegression(increasing=True, out_of_bounds="clip").fit(
                        norm_box_uncert[:, j][:split][
                            self.gt_classes[:split] == i
                        ].flatten(),
                        norm_residuals[:, j][:split][
                            self.gt_classes[:split] == i
                        ].flatten(),
                    )
                )
        ...
        for i in range(num_classes):
            for j in range(4):
                calib_norm_box_uncert[:, j][self.gt_classes[split:] == i] = calibrators[
                    i, j
                ].predict(norm_box_uncert[:, j][split:][self.gt_classes[split:] == i])
                self._conf_all(
                    self.gt_boxes[:, j][split:][self.gt_classes[split:] == i],
                    self.pred_boxes[:, j][split:][self.gt_classes[split:] == i],
                    norm_box_uncert[:, j][split:][self.gt_classes[split:] == i]
                    * norm[:, j][split:][self.gt_classes[split:] == i],
                    calib_norm_box_uncert[:, j][self.gt_classes[split:] == i]
                    * norm[:, j][split:][self.gt_classes[split:] == i],
                    0,
                )
                self._conf_all(
                    ...
                    * norm[:, j][split:][self.gt_classes[split:] == i],
                    0,
                    True,
                    method="per class relative IS-" + lbls[j] + " for class " + str(i),
                )
        ...
        for i in range(num_classes):
            for j in range(4):
                calibrators.append(
                    IsotonicRegression(increasing=True, out_of_bounds="clip").fit(
                        norm_box_uncert[:, j][self.gt_classes == i].flatten(),
                        norm_residuals[:, j][self.gt_classes == i].flatten(),
                    )
                )
        ...
        with open(
            self.saving_path
            + self.model_name
            + "/regres_models/regression_calib_iso_perclscoo_relative",
            "wb",
        ) as fp:
            pickle.dump(calibrators, fp)
    ...
    def box_uncert_calibration(self):
        ...
        with open(
            ...
            f.write("Measuring box uncertainty calibration" + " \n")

        if not os.path.exists(self.saving_path + self.model_name + "/regres_models"):
            os.makedirs(self.saving_path + self.model_name + "/regres_models")
        if not os.path.exists(self.saving_path + self.model_name + "/regres_plots"):
            os.makedirs(self.saving_path + self.model_name + "/regres_plots")

        split_factor = 0.8
        split = int(len(self.gt_boxes) * split_factor)
        self._absolute_calibration(split)
        self._relative_calibration(split)
        self._per_class_calibration(split)  # Always last
        print(
            "Plotted regression calibration on 20% of the validation dataset, the rest is used for fiting"
        )
