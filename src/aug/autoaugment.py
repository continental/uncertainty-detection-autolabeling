...
# ==============================================================================
#
# Modified Copyright (c) 2022-2024 Continental Automotive Technologies GmbH.
...
def _cutout_inside_bbox(image, bbox, pad_fraction):
  ...
  # Cutout mask will be size pad_size_heigh * 2 by pad_size_width * 2 if the region lies entirely within the bbox.
...
def distort_image_with_albumentations(image, bboxes, mode, available_ops, save_path):
  """ Apply augmentations from the albumentations library. 

  Args:
      image (tensor): Image to augment
      bboxes (tensor): Groundtruth bounding boxes, TODO in case augmentations with box transformations are added
      mode (string): Define parameter space; mode can be optimal, random or subjective
      available_ops (_type_): Define augmentation operations
      save_path (string): _description_

  Returns:
      Transformed image with the boxes
  """
  
  tf.logging.info('Using Albumentations.')

  if bboxes is None:
    bboxes = tf.constant(0.0)

  op_to_select = int(random.uniform(0, len(available_ops)))
  
  type_aug = available_ops[op_to_select]

  def transform_image(image_to_transform):
    """ Apply a Numpy augmentation function on an tensor image 

    Args:
        image_to_transform (tensor): Image to augment

    Returns:
        Augmented image
    """
    orig_shape = image_to_transform.shape
    if mode == "optimal":
        with open(save_path+type_aug+"/"+type_aug+"_opt_params", "rb") as fp: params = pickle.load(fp)

    if type_aug=="rain":      
        if mode == "random":
          bounds = [(0.0, 1.0), (0, 100), (1, 5), (1, 10), (-20,20)]
          params = [int(random.uniform(b[0], b[1])) for b in bounds]
        elif mode == "subjective":
          params = [0.8,20,1,10,4]      
        if params[2] > params[1]: params[2] == params[1]
        transform = A.Compose([A.RandomRain(brightness_coefficient=params[0], drop_length=int(np.round(params[1])), drop_width=int(np.round(params[2])), blur_value=int(np.round(params[3])), slant_lower=int(np.round(params[4])), slant_upper=int(np.round(params[4])), p=0.5)])
        ...
        if mode == "random":
          ...
          params = [3.0,0.4]
        transform = A.Compose([A.RandomSnow(brightness_coeff=params[0], snow_point_lower=params[1], snow_point_upper=params[1], p=0.5)])
        ...
        if mode == "random":
          ...
          params = [0.3,0.3]  
        transform = A.Compose([A.RandomFog(fog_coef_lower=params[0], fog_coef_upper=params[0], alpha_coef=params[1], p=0.5)])
        ...
        if mode == "subjective":
          params = [0.3,0.3] 
        transform = A.Compose([A.RandomBrightnessContrast(brightness_limit=(params[0],params[0]), contrast_limit=(params[1],params[1]), p=0.5)])

    elif type_aug == "eql":    
        if mode == "random":
          bounds = [(0.05, 100), (1, 100)]
          params = [int(random.uniform(b[0], b[1])) for b in bounds]
        elif mode == "subjective":
          params = [3,3]
        transform = A.Compose([A.CLAHE(clip_limit=(params[0],params[0]), tile_grid_size=(int(np.round(params[1])), int(np.round(params[1]))), p=0.5)])
        ...
        if mode == "random":
          ...
          params = [8,12,8]
        transform = A.Compose([A.HueSaturationValue(hue_shift_limit=(params[0],params[0]), sat_shift_limit=(params[1],params[1]), val_shift_limit=(params[2],params[2]), p=0.5)])

    else: 
      tf.logging.info('Unavailable augmentation operation')

    transformed_image = np.reshape(transform(image=image_to_transform.numpy())['image'],orig_shape)
    transformed_image = tf.convert_to_tensor(transformed_image)
    return transformed_image
  
  aug_image = tf.py_function(func=transform_image, inp=[image, ], Tout=tf.uint8)
  if aug_image.shape == None:
    logging.info("Image augmentation with albu unsuccessful")
    return (image, bboxes)
  else:
    logging.info("Image augmentation with albu successful")
