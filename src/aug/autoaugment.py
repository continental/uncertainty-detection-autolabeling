...
def distort_image_with_albumentations(image, bboxes, mode, available_ops, save_path):
    ...
    def transform_image(image_to_transform):
        ...
        if mode == "optimal":
            with open(
                save_path + type_aug + "/" + type_aug + "_opt_params", "rb"
            ) as fp:
        ...
        if type_aug == "rain":
            ...
            transform = A.Compose(
                [
                    A.RandomRain(
                        brightness_coefficient=params[0],
                        drop_length=int(np.round(params[1])),
                        drop_width=int(np.round(params[2])),
                        blur_value=int(np.round(params[3])),
                        slant_lower=int(np.round(params[4])),
                        slant_upper=int(np.round(params[4])),
                        p=0.5,
                    )
                ]
            )
            ...
            transform = A.Compose(
                [
                    A.RandomSnow(
                        brightness_coeff=params[0],
                        snow_point_lower=params[1],
                        snow_point_upper=params[1],
                        p=0.5,
                    )
                ]
            )
            ...
            transform = A.Compose(
                [
                    A.RandomFog(
                        fog_coef_lower=params[0],
                        fog_coef_upper=params[0],
                        alpha_coef=params[1],
                        p=0.5,
                    )
                ]
            ...
            transform = A.Compose(
                [
                    A.RandomBrightnessContrast(
                        brightness_limit=(params[0], params[0]),
                        contrast_limit=(params[1], params[1]),
                        p=0.5,
                    )
                ]
            )
            ...
            transform = A.Compose(
                [
                    A.CLAHE(
                        clip_limit=(params[0], params[0]),
                        tile_grid_size=(
                            int(np.round(params[1])),
                            int(np.round(params[1])),
                        ),
                        p=0.5,
                    )
            ...
            transform = A.Compose(
                [
                    A.HueSaturationValue(
                        hue_shift_limit=(params[0], params[0]),
                        sat_shift_limit=(params[1], params[1]),
                        val_shift_limit=(params[2], params[2]),
                        p=0.5,
                    )
                ]
            )
        ...
        transformed_image = np.reshape(
            transform(image=image_to_transform.numpy())["image"], orig_shape
        )
