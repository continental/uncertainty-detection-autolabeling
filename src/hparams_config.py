...
def default_detection_configs():
  ...
  h.infer_draw_uncert = True # Save an image with the uncertainty for each predicted image
  ...
  h.clip_max_uncert = 1024 # Clip uncertainty during training if values exceed (input image size)  
  ...
  h.mc_classheadrate = 0.0  # Setting it while mcdropoutrate=0 will activate only for class head
  h.mc_boxheadrate = 0.0  # Setting it while mcdropoutrate=0 will activate only for box head
  ...
  h.calib_method_class = "iso_percls" #["ts_all", "ts_percls", "iso_all", "iso_percls"] 
  ...
  h.count_classes = False # Prints and plots class distribution for the dataset before training
  ...
  h.save_train_images = False # Save input images as they are fed to the model to tensorboard
  ...
  h.albumentations_ops = ["rain","snow","fog","sat"] #"brct", "eql" # Augmentation operations
  ...
  h.thr_iou_thrs = list(np.round(np.arange(0.5,0.76,0.05), 2)) # Thresholding ious used for optimization and metrics calculation, default to np.arange(0.5,0.76,0.05)
