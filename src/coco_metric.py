...
import label_util
from custom_cocoeval import COCOeval_all
...
class EvaluationMetric():
  ...
  def __init__(self, filename=None, testdev_dir=None, label_map=None , apiou_curve=True):
    ...
    Args:
      ...
      apiou_curve: a bool to select if values should be calculated for extended thresholds from 0 to 1
    """
  ...
  def evaluate(self, log_level=tf.compat.v1.logging.INFO):
    ...
    if self.testdev_dir:
      ...
      if self.apiou_curve:
        all_eval =  COCOeval_all(coco_gt, coco_dt, iouType='bbox')
        all_eval.params.imgIds = image_ids
        all_eval.evaluate()
        all_eval.accumulate()
        precision_all = all_eval.eval['precision'][:, :, :, 0, -1]
      ...
      if self.label_map:
        ...
        for c in range(precision.shape[-1]):  # iterate over all classes
          ...
          ap_perclass[int(self.category_ids[c])-1] = ap_c #-1 because they start at 1
        coco_metrics = np.concatenate((coco_metrics, ap_perclass))
      # Return the concat normal and per-class AP.
      if self.apiou_curve:
        return np.array(coco_metrics, dtype=np.float32), precision_all
      else:
  ...
  def result(self, log_level=tf.compat.v1.logging.INFO):
    ...
    if self.metric_values is None:
      if self.apiou_curve:
        self.metric_values, pall = self.evaluate(log_level)
        return self.metric_values, pall
      else:
