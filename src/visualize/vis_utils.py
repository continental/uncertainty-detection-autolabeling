...
def draw_bounding_box_on_image_array(image,
  ...
  Args:
    ...
    uncertainty_norm: normalized uncertainty for coloring
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  draw_bounding_box_on_image(image_pil, ymin, xmin, ymax, xmax, color,
                             thickness, display_str_list,
                             use_normalized_coordinates, uncertainty, uncertainty_norm)
...
def draw_bounding_box_on_image(image,
  ...
  if uncertainty is not None:    
    # Draw the bounding box with color and alpha based on its uncertainty
    cmap = plt.cm.jet
    bbox_color = cmap(uncertainty_norm(uncertainty))
    bbox_color = tuple((np.asarray([bbox_color[0], bbox_color[1], bbox_color[2], 0.5])*255).astype(int)) # alpha
    draw.rectangle([(left, top), (right, bottom)], outline=None, fill=bbox_color)
...
def visualize_boxes_and_labels_on_image_array(
  ...
  Args:
    ...
    uncertainty: array of predicted uncertainty
  ...
  if uncertainty is not None: filtered_uncertainty = []
  ...
  for i in range(boxes.shape[0]):
    ...
    if scores is None or scores[i] > min_score_thresh:
      ...
      if uncertainty is not None: # Filter uncertainties accordingly
        filtered_uncertainty.append(uncertainty[i])
      if instance_masks is not None:
  ...
  # Draw uncertainty on mask
  if uncertainty is not None: 
    uncertainty = np.asarray(filtered_uncertainty)  
    norm = mat_colors.Normalize(vmin=min(uncertainty), vmax=max(uncertainty))
  else: 
    uncertainty = [None]*len(box_to_color_map)
    norm = None

  # Draw all boxes onto image.
  for (box, color), uncert in zip(box_to_color_map.items(), uncertainty):  
  ...
  if norm is not None:
    ...
    plt.close(fig)
    buf.seek(0)
    return np.asarray(tf.image.decode_png(buf.getvalue(), channels=4))   
  else:
    	return image
