...
from utils_box import calc_rmse
from utils_extra import mtplt_to_img, plot_conf_matrix, plot_roc, mc_eval
...
class EarlyStoppingCustomCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if current_metric is None:
            raise ValueError(f"Metric '{self.monitor}' not found in logs. Available metrics are: {','.join(logs.keys())}")
...
class COCOCallback(tf.keras.callbacks.Callback):
  ...
  def _get_detections(self, images, labels):    
    if self.config.mc_dropout: cls_outputs, box_outputs = mc_eval(self.model, images, self.config)
    else: cls_outputs, box_outputs = self.model(images, training=False)
    ...
    if self.conf_matrix:
      if self.config.enable_softmax:
        tf.numpy_function(self._gather_pred_classes, [detections[:,:,-(self.config.num_classes+1)]], [])
        tf.numpy_function(self._gather_pred_logits, [detections[:,:,-self.config.num_classes:]], [])
        
      else:
        tf.numpy_function(self._gather_pred_classes, [detections[:,:,-1]], [])      
      
      tf.numpy_function(self._gather_pred_boxes, [detections[:,:,1:5]], [])
      tf.numpy_function(self._gather_gt_boxes, [labels['groundtruth_data'][:,:,:4]], [])
  ...
  def on_epoch_end(self, epoch, logs=None):
    ...
    if self.update_freq and epoch % self.update_freq == 0:
      ...
      if self.conf_matrix:
        ...
        gtbboxes = np.swapaxes(np.swapaxes([gtbboxes[:,:,1], gtbboxes[:,:,0], gtbboxes[:,:,3], gtbboxes[:,:,2]], 0,2), 0,1)
        bboxes = self.gather_boxes.reshape([-1,100,4])

        valid_indices = np.all(gtbboxes > -1, axis=-1)
        best_indices = [list(map(lambda j: gt_box_assigner("IoU", gtbboxes[i], bboxes[i], j), np.where(valid_indices[i])[0])) for i in range(len(gtbboxes))]

        filtered_bboxes = np.concatenate([bboxes[i][best_indices[i]] for i in range(len(bboxes))])
      ...
      iou_thrs = np.linspace(.05, 0.95, int(np.round((0.95 - .05) / .05)) + 1, endpoint=True)
      map_plots = []
      ...
      try:
        aps_prothr_percls = [[np.mean(all_maps[i,:,j]) for i in range(len(all_maps))] for j in range(self.config.num_classes)]    
        map_fig = self._plot_map(aps_prothr_percls, iou_thrs, per_cls=True)
        map_plots.append(mtplt_to_img(map_fig))
      except IndexError: # Pop from empty list error
        pass
      with self.file_writer.as_default():
        tf.summary.image("AP/IOU Curves", tf.concat(map_plots,axis=0), step=epoch, max_outputs=1000)  
      ...
      if self.conf_matrix:    
        ...
        for i in range(len(labels)):          
          gt_classes_filter.append(labels[i]['groundtruth_data'][:,:,-1]) # [y1, x1, y2, x2, is_crowd, area, class]
        ...
        if self.config.enable_softmax:
          pred_softmax = stable_softmax(self.gather_logits.reshape(-1, self.config.num_classes)[gt_classes_filter>=0])
...
class DisplayCallback(tf.keras.callbacks.Callback):
  ...
  def __init__(self, image_names, update_freq=None):
    ...
    """
    super().__init__()     
    self.sample_images = [tf.image.decode_jpeg(tf.io.read_file(im), channels=3) for im in image_names] 
    self.update_freq = update_freq
  ...
  def _draw_inference(self, step):
    ...
    for i in range(len(self.iou_thrs)):
        self.model.config.override('nms_configs.iou_thresh='+str(self.iou_thrs[i]))  
        for j in range(len(self.score_thrs)):  
            self.model.config.override('nms_configs.score_thresh='+str(self.score_thrs[j]))
            images = []
            for k in range(len(self.sample_images)):
                predictions = self.model(tf.expand_dims(self.sample_images[k], axis=0), training=False)
                if self.model.config.enable_softmax:
                  boxes, scores, classes, valid_len, _ = tf.nest.map_structure(np.array, predictions)
                else:
                  boxes, scores, classes, valid_len = tf.nest.map_structure(np.array, predictions)
                filter = valid_len[0]
            ...
            with self.file_writer.as_default():
                for k in range(len(images)):
                    tf.summary.image(name='Test image for threshold, iou: '+str("%.1f" % self.iou_thrs[i])+', score: '+str("%.1f" % self.score_thrs[j])+"/"+str(k), data=tf.expand_dims(images[k], axis=0), step=step, max_outputs=10000000)
    # Reset
...
class KeepLastNCheckpoints(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if epoch > 3:
          checkpoint_files = glob.glob(os.path.join(self.directory, 'best_ckpt-*.index'))
          ...
          if len(checkpoint_files) > self.num_to_keep:
              for file_to_remove in checkpoint_files[:-self.num_to_keep]:
                  ...
                  os.remove(file_to_remove.split(".index")[0]+".data-00000-of-00001")

def get_callbacks(params, val_dataset=None):
  """Get callbacks for given params."""
  if params['moving_average_decay']:
    from tensorflow_addons import callbacks as tfa_callbacks # pylint: disable=g-import-not-at-top
    ...
    best_ckpt_callback = tf.keras.callbacks.ModelCheckpoint(
        ...
        mode='min',
        save_weights_only=True)    
    keep_last_n_checkpoints = KeepLastNCheckpoints(directory=params['model_dir']+"/best_ckpts/", num_to_keep=3)
  ...
  if params['early_stopping_patience'] > 0: callbacks.append(EarlyStoppingCustomCallback(patience=params['early_stopping_patience']))
  return callbacks
...
class BoxLoss(tf.keras.losses.Loss):
  ...
  def call(self, y_true, box_outputs):
    if self.loss_att:
      ...
      factor = tf.math.square(loss_att_outputs[:,:,:,int(loss_att_outputs.shape[3]/2):])/2.  # Take only uncert of width and height
    ...
    mask = tf.cast(box_targets != 0.0, box_outputs.dtype) # number of acutal bbs (1,0,1,1,1,0)
    ...
    if self.ls_type == "huber":
      ...
      box_loss = tf.cast(self.mse(box_targets, box_outputs),
                        box_outputs.dtype)
    ...
    if self.loss_att:
      ...
      return 0.25*tf.reduce_sum((box_loss/loss_att_outputs + tf.math.log(1+loss_att_outputs))*mask)/normalizer
    else: 
...
class ConfusionMatrixMetric(tf.keras.metrics.Metric):
    ...
    def __init__(self, num_classes, label_map, **kwargs):
        ...
        if label_map is not None: self.class_names = np.asarray([label_map[key] for key in label_map])
        else: self.class_names = np.arange(1, num_classes+1)
        self.total_conf_matrix = self.add_weight("total", shape=(self.num_classes, self.num_classes), initializer="zeros")
    ...
    def confusion_matrix(self,y_true, y_pred):
        ...
        conf_matrix = tf.math.confusion_matrix(y_true, y_pred, dtype=tf.float32, num_classes=self.num_classes)
        return conf_matrix
    ...
    def _calc_confusion_matrix(self):
        ...
        f1 = 2*tf.math.divide_no_nan(tf.multiply(precision, recall), tf.add(precision, recall))
...
class EfficientDetNetTrain(efficientdet_keras.EfficientDetNet):
  ...
  def __init__(self, *args, **kwargs):
    ...
    if self.config.loss_attenuation:
      self.metrics_collect = {"class": [ConfusionMatrixMetric(self.config.num_classes, self.config.label_map)], "box": [RmseMetric(), MeanUncertMetric(), StdUncertMetric()]}
    else:
      self.metrics_collect = {"class": [ConfusionMatrixMetric(self.config.num_classes, self.config.label_map)], "box": [RmseMetric()]}
  ...
  def _get_class_data(self, labels, cls_outputs):
    ...
    for level in levels:
      ...
      y_true_level = tf.cast(tf.reshape(labels['cls_targets_%d' % (level + self.config.min_level)], [-1]), dtype= y_pred_level.dtype)
    ...
    y_pred = tf.reshape(tf.gather(y_pred, mask, axis=0), [-1, self.config.num_classes])
  ...
  def _get_box_data(self, labels, box_outputs, mask):
    ...
    for level in levels:
      if self.config.loss_attenuation:
          box_pred = box_outputs[level][:,:,:,:int(box_outputs[0].shape[-1]/2)]
          uncert_pred = box_outputs[level][:,:,:,int(box_outputs[0].shape[-1]/2):]
      ...
      y_true_level = tf.cast(tf.reshape(labels['box_targets_%d' % (level + self.config.min_level)], [-1, 4]), dtype= y_pred_level.dtype)
      y_pred.append(y_pred_level)
      y_true.append(y_true_level)
    ...
    if self.config.loss_attenuation:
      y_uncert = tf.reshape(tf.gather(tf.concat(y_uncert, axis=0), mask, axis=0), [-1, 4])
  ...
  def _get_metrics(self, labels, cls_outputs, box_outputs):
    ...
    [class_metric.update_state(y_true_cls, y_pred_cls) for class_metric in self.metrics_collect["class"]]
    y_true_box, y_pred_box = self._get_box_data(labels, box_outputs, mask_pos)
    [box_metric.update_state(y_true_box, y_pred_box) for box_metric in self.metrics_collect["box"]]
    metrics_output = {}
    for i in range(len(self.metrics_collect["class"])):
      ...
      else: metrics_output[m.name]= m.result()    
  ...
  def _clip_uncert(self, box_outputs):
    ...
    for level in range(len(box_outputs)):
      loss_att_values = tf.clip_by_value(t=box_outputs[level][:,:,:,int(box_outputs[0].shape[-1]/2):], clip_value_min=self.config.clip_min_uncert, clip_value_max=self.config.clip_max_uncert)
      box_outputs[level] = tf.concat([box_outputs[level][:,:,:,:int(box_outputs[0].shape[-1]/2)], loss_att_values], axis=-1)
    return box_outputs
  ...
  def test_step(self, images, labels):
    ...
    loss_vals = {}
    if 'object_detection' in self.config.heads:
      # attenuate and clip uncertainty, save attenuation factor
      if self.config.loss_attenuation:
        box_outputs = self._clip_uncert(box_outputs)
...
class CollectEpochLoss:
    ...
    def update(self, loss_vals, val=False):
      if val: 
        ...
        val = ""        
        self.n_train += 1              
      self.epochend = {val+i: self.epochend[val+i]+loss_vals[i] if val+i in self.epochend else loss_vals[i] for i in loss_vals}
