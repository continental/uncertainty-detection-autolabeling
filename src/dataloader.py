...
import utils
import anchors
from object_detection_efficientdet import preprocessor
from object_detection_efficientdet import tf_example_decoder
    ...
    with tf.name_scope('parser'):
      ...
      if self._is_training:
        ...
        if params.get('autoaugment_policy', None):
          ...
          elif params['autoaugment_policy'] == 'albu': # Added custom augmentation      
            image, boxes = autoaugment.distort_image_with_albumentations(
                image, boxes, params['albumentations_mode'], 
                params['albumentations_ops'], params['albumentations_path'])
          else:
  ...
  def __call__(self, params, input_context=None, batch_size=None, names=False):
    ...
    if names:
        ...
        for raw_record in dataset.as_numpy_iterator(): 
            ...
            img_names.append(str(example.features.feature["image/filename"].bytes_list.value[0])[2:-1])
