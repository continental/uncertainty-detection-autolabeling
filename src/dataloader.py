...
import anchors
import utils
from object_detection_efficientdet import preprocessor, tf_example_decoder
...
class InputReader:
    ...
    def __init__(
        ...
        debug=False,
        activate_pseudo_score=False,  # Adds a score per detection/image to the GT data
    ):
    ...
    def dataset_parser(self, value, example_decoder, anchor_labeler, params):
        ...
        Returns:
          ...
          pseudo_score: Pseudo-labels confidence scrore. The tensor is padded with -1
            to the fixed dimension [self._max_instances_per_image]. Opttional.
        ...
        with tf.name_scope("parser"):
            ...
            if self.activate_pseudo_score:
                pseudo_score = data["groundtruth_pseudo_score"]
            ...
            if self._is_training:
                ...
                if params.get("autoaugment_policy", None):
                    ...
                    if params["autoaugment_policy"] == "randaug":
                        ...
                        params["autoaugment_policy"] == "albu"
                    ):  # Added custom augmentation
                        image, boxes = autoaugment.distort_image_with_albumentations(
                            image,
                            boxes,
                            params["albumentations_mode"],
                            params["albumentations_ops"],
                            params["albumentations_path"],
                        )
                    else:
            ...
            if self.activate_pseudo_score:
                pseudo_score = pad_to_fixed_size(
                    pseudo_score, -1, [self._max_instances_per_image, 1]
                )
    ...
    def process_example(
        ...
        gt = [boxes, is_crowds, areas, classes]
        if pseudo_score is not None:
            gt.append(pseudo_score)
        groundtruth_data = tf.concat(gt, axis=2)
    ...
    def __call__(self, params, input_context=None, batch_size=None, names=False):
        ...
        example_decoder = tf_example_decoder.TfExampleDecoder(
            ...
            activate_pseudo_score=self.activate_pseudo_score,
        )
