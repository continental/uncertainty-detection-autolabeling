...
class EpistemicVSAleatoric:
    ...
    def __init__(
        self, model_name, im_numbs=10, iou_thr=0.1, uncert="uncalib", ent=False
    ):
    ...
    def _save_crops(self, img_names, coords, label):
        ...
        if num_images > 0:
            ...
            for i in range(num_images):
                ...
                im = Image.open(
                    PARENT_DIR + "/datasets/" + self.img_source_path + img_names[i]
                )
    ...
    def plot_unc_metrics(self, data_list, labels, colors):
        ...
        for i, (data, label, color) in enumerate(zip(data_list, labels, colors)):
            ...
            y = [
                np.count_nonzero((np.equal(cp, j)))
                for j in range(len(self.class_names))
            ]

            axes[0, 1].bar(
                np.arange(len(self.class_names)) - 0.2,
                y,
                width=0.2,
                align="edge",
                label="Pred. " + label,
    ...
    def get_metrics_comp(self, filtered_detections, img_names, bbox, gt_bbox):
        ...
        data.append(
            [
                self.class_names[int(filtered_detections[i]["class"] - 1)]
                for i in range(len(filtered_detections))
            ]
        )
        data.append(
            [
                self.class_names[int(filtered_detections[i]["gt_class"] - 1)]
                for i in range(len(filtered_detections))
            ]
        )
        data.append(
            [filtered_detections[i]["score"] for i in range(len(filtered_detections))]
        )
        data.append(
            [filtered_detections[i]["entropy"] for i in range(len(filtered_detections))]
        )
        data.append(
            (np.stack(bbox)[:, 3] - np.stack(bbox)[:, 1])
            * (np.stack(bbox)[:, 2] - np.stack(bbox)[:, 0])
        )
        data_brisq = []
                    ...
                    obj.score(
                        np.asarray(
                            Image.open(
                                PARENT_DIR
                                + "/datasets/"
                                + self.img_source_path
                                + img_names[i]
                            ).crop((bbox[i][1], bbox[i][0], bbox[i][3], bbox[i][2]))
                        )
                    )
                )
            )
    ...
    def get_plot_grid(self, av_al, av_mc):
        ...
        min_samples = 3
        cell_indices, x_min, x_step, y_min, y_step = self._get_cell_indices(
            self.grid_size, uncert_data
        )
        top_left_empty = (
            len(cell_indices[cell_indices == self.grid_size**2 - self.grid_size])
            < min_samples
        )
        bottom_right_empty = (
            len(cell_indices[cell_indices == self.grid_size - 1]) < min_samples
        )
        ...
        if top_left_empty == False or bottom_right_empty == False:
            while top_left_empty == False or bottom_right_empty == False:
                ...
                top_left_empty = (
                    len(
                        cell_indices[cell_indices == self.grid_size**2 - self.grid_size]
                    )
                    < min_samples
                )
                bottom_right_empty = (
        ...
        plt.scatter(
            uncert_data[:, 0],
            uncert_data[:, 1],
            c=[cell_colors[i] for i in cell_indices],
        )
        ...
        if self.ent:
            plt.title("Distribution Loc AL vs cls ENT")
        else:
            plt.title("Distribution Localization AL vs EP")
    ...
    def compare_epal(self):
        ...
        uncert_albox = np.nan_to_num(
            [det[self.uncert + "_albox"] for det in detections]
        )
        ...
        if self.ent:
            ...
            uncert_mcbox = np.nan_to_num(
                [det[self.uncert + "_mcbox"] for det in detections]
            )
        ...
        lal_hep_ind = np.where(cell_indices == self.grid_size**2 - self.grid_size)[0][
            : self.numb
        ]
        ...
        # Gather image names
        self.imgs_hal_lep = [
            detections[hal_lep_ind][i]["image_name"]
            for i in range(len(detections[hal_lep_ind]))
        ]
        self.imgs_lal_hep = [
            detections[lal_hep_ind][i]["image_name"]
            for i in range(len(detections[lal_hep_ind]))
        ]
        ...
        if len(hal_lep_ind) > 0:
            metrics_hal_lep = self.get_metrics_comp(
                detections[hal_lep_ind],
                self.imgs_hal_lep,
                bbox[hal_lep_ind],
                gt_bbox[hal_lep_ind],
            )
            available_metrics += [
                metrics_hal_lep + [av_mc[hal_lep_ind]] + [av_al[hal_lep_ind]]
            ]
            available_labels += ["High Al-Low EP"]
            available_colors += ["green"]
        if len(lal_hep_ind) != 0:
            metrics_lal_hep = self.get_metrics_comp(
                detections[lal_hep_ind],
                self.imgs_lal_hep,
                bbox[lal_hep_ind],
                gt_bbox[lal_hep_ind],
            )
            available_metrics += [
                metrics_lal_hep + [av_mc[lal_hep_ind]] + [av_al[lal_hep_ind]]
            ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Command-line tool to compare Epistemic vs Aleatoric uncertainty"
    )
    ...
    parser.add_argument(
        "--uncert", type=str, help="Uncertainty type", default="uncalib"
    )
    parser.add_argument("--ent", type=bool, help="Select entropy", default=False)

    args = parser.parse_args()
    if args.model_name is not None:
        EpistemicVSAleatoric(
            args.model_name, args.im_numbs, args.iou_thr, args.uncert, args.ent
        ).compare_epal()
    else:
        ...
        try:
            model_name = str(
                input(
                    "Enter model name, e.g., EXP_KITTI_allclasses_mcdropout_lossattVhead.3_ckpt20: "
                )
            )
            im_numbs = int(
                input("Enter limit on number of images, default is 10: ") or 10
            )
            iou_thr = float(
                input("Enter iou threshold for filtering, default is 0.1: ") or 0.1
            )
            uncert = str(
                input("Enter uncert type, default is 'uncalib': ") or "uncalib"
            )
            ent = bool(
                input("Enter if entropy to be used, default is False: ") or False
            )
            EpistemicVSAleatoric(
                model_name, im_numbs, iou_thr, uncert, ent
            ).compare_epal()
        except ValueError:
