...
class MBConvBlock(tf.keras.layers.Layer):
  ...
  def __init__(self, block_args, global_params, name=None, extra_config=None):
    ...
    Args:
      ...
      name: layer name.
      extra_config: Dictionary of hparams detector config.
    ...
    try: 
      self.mc_dropout = extra_config["mc_dropout"] 
      self.mc_dropoutrate = extra_config["mc_dropoutrate"] 
    except:
    ...
    def _call(inputs):
      ...
      if self._block_args.fused_conv:
        ...
        if self._block_args.expand_ratio != 1:
          ...
          if self.mc_dropout: x = tf.keras.layers.SpatialDropout2D(self.mc_dropoutrate)(x, training=True)  #add mc after depthwise activationa
...
class Model(tf.keras.Model):
  ...
  def __init__(self, blocks_args=None, global_params=None, name=None, extra_config=None):
  ...
  def _build(self):
    ...
    for i, block_args in enumerate(self._blocks_args):
      ...
      if not block_args.super_pixel:  #  no super_pixel at all
        self._blocks.append(
            conv_block(block_args, self._global_params, name=block_name(), extra_config=self.extra_config))
      else:
