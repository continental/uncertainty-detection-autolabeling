...
class ClassificationCalib:
    ...
    def __init__(self, y_true, y_pred, ep_unc, model_name, general_path):
        ...
        self.orig_ytrue = np.squeeze(np.eye(num_classes)[y_true.astype(int).reshape(-1)])
    ...
    def iso_regression(self):
        ...
        """
        if self.calib_percls: isos = [IsotonicRegression(y_min = 0, y_max = 1, out_of_bounds = 'clip').fit(self.y_pred[:,i], self.y_true[:,i]) for i in range(len(self.classes_label))]      
        else: isos = IsotonicRegression(y_min = 0, y_max = 1, out_of_bounds = 'clip').fit(self.y_pred.flatten(), self.y_true.flatten())     
        return isos 
    ...
    def temp_scaling(self):
        ...
        Returns:
            Temperature/s
        """
        if self.calib_percls: x = tf.Variable([1.0]*len(self.classes_label), trainable=True, dtype=tf.float32)
        else: x = tf.Variable(1.0, trainable=True, dtype=tf.float32)

        @tf.autograph.experimental.do_not_convert
        def f_x():
            """ Loss function to be reduced with gradient descent
            """
            return tf.keras.losses.CategoricalCrossentropy(from_logits=True)(self.y_true, tf.divide(self.y_pred, x))
    ...
    def _calibr_bins(self, y_true, y_pred):
        ...
        self.ece = np.sum(np.abs(prob_true - prob_pred) * (bin_total[nonzero] / len(ytrue)))
    ...
    def _bincount_bars(self, ax, color="blue", label="", alpha=1.0):
        ...
        w = [self.bin_prob[i] - self.bin_prob[i - 1] for i in range(1, len(self.bin_prob))]
        centers = [(self.bin_prob[i] + self.bin_prob[i - 1]) / 2 for i in range(1, len(self.bin_prob))]
        ax.bar(centers, self.bin_count, linewidth=1, width = w,edgecolor="orange", color=color, label=label, alpha=alpha)
    ...
    def _calibr_bars(self, ax, colors=["skyblue", "crimson"], alpha=1.0):
        ...
        ax.bar(centers, self.freq, linewidth=1, width=w, edgecolor="black",color=colors[0], label=l1, alpha=alpha)
        ax.bar(self.prob, self.freq - self.prob, bottom=self.prob, width=0.005, color=colors[1], label=l2,
               alpha=alpha)
    ...
    def _calibr_lines(self, ax, colors, labels, alpha=1.0):
        ...
        """
        ax.plot(self.prob, self.freq, "s-", color=colors[0], label=labels[0], alpha=alpha)
        ax.fill_between(self.prob, self.freq, self.prob, color=colors[1], label=labels[1], alpha=alpha)
        ax.set_xlabel('Predicted Probability')
        ax.set_ylabel('Proportion of Positives')
        ax.legend(prop={'size':8},frameon=False,handletextpad=0.2) 
        ax.set_ylim([-0.05, 1.05])

    def _calibr_plot(self, quantilestrategy=False, calib=False, T=False, split=0, calib_percls=False):
        ...
        self.nll =  tf.keras.losses.CategoricalCrossentropy()( self.orig_ytrue[split:], self.orig_ypred[split:]).numpy()
        if calib:
            self.y_pred=self.orig_ypred[:split]
            self.y_true=self.orig_ytrue[:split]
            if T: 
                self.temp_scaling()
                self.y_pred = stable_softmax(self.orig_ypred[split:] / self.temper) # Necessary to recalibrate into sum 1, take val split
            else:
            ...
            if self.calib_percls: 
                if T: 
                    ...
                    ycalibs = [iso[i].predict(self.y_pred[:,i]) for i in range(len(self.classes_label))]
                    ycalibs = np.stack(ycalibs,axis=1)
                    self.y_pred = ycalibs/np.stack([np.sum(ycalibs, axis=-1)]*len(self.classes_label),axis=-1)
                ...
                if T:                
                    ...
                    ycalibs = iso.predict(self.y_pred.flatten()).reshape(self.y_pred.shape)
                    self.y_pred = ycalibs/np.stack([np.sum(ycalibs, axis=-1)]*len(self.classes_label),axis=-1)       
        else:
        ...
        self.calib_nll =  tf.keras.losses.CategoricalCrossentropy()( self.y_true, self.y_pred).numpy()
        ...
        for target in targets:
            ...
            self._calibr_bins(y_true=self.y_true[:, target], y_pred=self.y_pred[:, target].astype("float64"))
            ...
            f1.set_title(err + str(np.around(self.ece,3)) + "\nMCE " + str(np.around(self.mce,3)) + "     Brier: " +str(np.around(brier,3)))
        
        fig.tight_layout()
        plt.savefig(self.saving_path+self.model_name+'/classification/class_plot_'+calib_methd+'.png',bbox_inches='tight')                      
        ...
        if calib: nll = self.calib_nll
        else: nll = self.nll
        fin_1.set_title(err + str(np.around(self.ece,3))+ "     NLL: "+str(np.around(nll,3))+ "     SCE: "+ str(np.around(np.mean(eces),3)) + "\n" + "MCE " + str(np.around(self.mce,3)) + "     Brier: " +str(np.around(brier,3)))
        if calib and T: 
            try: len(self.temper)
            except: self.temper = np.asarray([self.temper])
        ...
        fig.tight_layout()
        plt.savefig(self.saving_path+self.model_name+'/classification/all_plot_'+calib_methd+'.png',bbox_inches='tight')                      
    ...
    def class_calibration(self):
        ...
        def _generate_calib(y_true, y_pred, save_name=""):
            ...
            with open(self.saving_path+self.model_name+"/classification/"+save_name+"classification_ts_all", "wb") as fp: pickle.dump(temps,fp)
            
            self.calib_percls = True
            temps = self.temp_scaling()
            with open(self.saving_path+self.model_name+"/classification/"+save_name+"classification_ts_percls", "wb") as fp: pickle.dump(temps,fp)
            ...
            with open(self.saving_path+self.model_name+"/classification/"+save_name+"classification_iso_all", "wb") as fp: pickle.dump(isos,fp)
            self.calib_percls = True
            isos = self.iso_regression()
            with open(self.saving_path+self.model_name+"/classification/"+save_name+"classification_iso_percls", "wb") as fp: pickle.dump(isos,fp)
        ...
        if self.ep_unc is not None:        
            logits_dist = tfp.distributions.Normal(loc=self.orig_ypred, scale=self.ep_unc)
            samples = logits_dist.sample(10)
            sample_ypred = np.asarray(samples).reshape([-1,self.orig_ypred.shape[-1]])
            sample_ytrue = np.stack([self.orig_ytrue]*10,axis=0).reshape([-1,self.orig_ypred.shape[-1]])      
            _generate_calib(sample_ytrue, sample_ypred, save_name="unc_")
        
        
        print("Plotted classification calibration on 20% of the validation dataset, the rest is used for fiting")
