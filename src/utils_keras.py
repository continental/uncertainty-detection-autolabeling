...
""" Modified. Originally named util_keras.py.
Common keras utils."""
...
HUB_CPT_NAME = collections.OrderedDict(
    [
        ("class_net/class-predict/", "classes"),
...
def restore_ckpt(
    model, ckpt_path_or_file, ema_decay=0.9998, skip_mismatch=True, exclude_layers=None
):
    ...
    if var_list[0][0] == "_CHECKPOINTABLE_OBJECT_GRAPH":
        ...
        var_dict = {var.name[: -len(":0")]: var for var in ema_vars.values()}
        ...
        if exclude_layers:
            if (
                var_list[0][0] == "efficientnet-b0/blocks_0/conv2d/kernel"
            ):  # If backbone only pre-trained
                exclude_layers = set(exclude_layers)
            else:
                exclude_layers = set(
                    [exclude_layers[0]]
                )  # If whole detector, skip class head, as number of clases differ (check train.py)
        else:
            exclude_layers = []
