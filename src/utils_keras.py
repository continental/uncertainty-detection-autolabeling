...
def restore_ckpt(model,
  ...
  if (var_list[0][0] ==
    ...
    if exclude_layers:
      if (var_list[0][0] == 'efficientnet-b0/blocks_0/conv2d/kernel'): # If backbone only pre-trained
            exclude_layers = set(exclude_layers)
      else:
            exclude_layers = set([exclude_layers[0]]) # If whole detector, skip class head, as number of clases differ (check train.py)
    else:
        exclude_layers = []    
