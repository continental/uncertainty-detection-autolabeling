    ...
    with ds_strategy.scope():
        ...
        def model_fn(images, labels):
            if config.mc_dropout:
                cls_outputs, box_outputs = mc_eval(
                    model, images, config
                )  # If mc dropout is activated
            else:
            ...
            tf.numpy_function(
                evaluator.update_state, [labels["groundtruth_data"], detections], []
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=int, choices=range(len(datasets)), help="Select a dataset"
    )
