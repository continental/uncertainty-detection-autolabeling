...
def apply_corruption(mode, image):
    ...
    for i in x_ax:
        ...
        if mode == "br" or mode == "ct":
            noisy_img = cv2.addWeighted(image, alpha, np.zeros(image.shape, image.dtype),0, beta)
        elif mode == "bl":
            ...
            noisy_img[cent[0]-halfsize:cent[0]+halfsize,cent[1]-halfsize:cent[1]+halfsize] += gaussian
            noisy_img = (noisy_img*255/noisy_img.max()).astype(np.uint8)
        elif mode == "mb":
            kernel_size=i
...
def add_weather(image, weather_condition, val1=None, val2=None, val3=None):
    ...
    if weather_condition == 'fog':
        transform = A.Compose([A.RandomFog(fog_coef_lower=val1 or 0.2, fog_coef_upper=val2 or 0.4, alpha_coef=val3 or 0.5, p=1)])
        augmented_image = transform(image=image)['image']
    elif weather_condition == 'rain':
        transform = A.Compose([A.RandomRain(brightness_coefficient=val1 or 0.9, drop_width=val2 or 1, blur_value=val3 or 5, p=1)])
        augmented_image = transform(image=image)['image']
    elif weather_condition == 'snow':
        transform = A.Compose([A.RandomSnow(brightness_coeff=val1 or 2.5, snow_point_lower=val2 or 0.3, snow_point_upper=val3 or 0.5, p=1)])
        augmented_image = transform(image=image)['image']
