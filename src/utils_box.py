...
def calc_iou_np(gt_boxes, pred_boxes):
    ...
    interArea = np.maximum(0.0, (xB-xA)) * np.maximum(0.0, (yB-yA)).astype(np.float64)
    boxAarea = np.abs(gt_boxes[:,3]-gt_boxes[:,1]) * np.abs(gt_boxes[:,2]-gt_boxes[:,0]).astype(np.float64)
    boxBarea = np.abs(pred_boxes[:,3]-pred_boxes[:,1]) * np.abs(pred_boxes[:,2]-pred_boxes[:,0]).astype(np.float64)
    
    iou = np.divide(interArea,(boxAarea+boxBarea - interArea), out=np.zeros_like(interArea), where=(boxAarea+boxBarea - interArea)!=0)
    return iou
...
def calc_rmse(gt_boxes, pred_boxes):
  """ Calculate RMSE
    ...
    """
  return tf.math.sqrt(tf.reduce_mean(tf.math.square(pred_boxes - gt_boxes)[gt_boxes!=0.0]))
...
def decode_uncert(pred_boxes, box_uncert, anchor_boxes, method="l-norm", n_samples=30):
  ...
  if method == "l-norm":
    ...
    n_dist = tfp.distributions.MultivariateNormalDiag(loc=[ty, tx, th, tw], scale_diag=tf.math.sqrt([dty, dtx, dth, dtw]))
    ...
    dycenter, dxcenter, dh, dw  = t_n_y.variance(), t_n_x.variance(), t_n_h.variance(), t_n_w.variance()
    ...
    dymin = tf.abs(dycenter - dh / 2.)
    dxmin = tf.abs(dxcenter - dw / 2.)
    dymax = dycenter + dh / 2.
    dxmax = dxcenter + dw / 2.


  coords = tf.cast(tf.stack([ymin, xmin, ymax, xmax], axis=-1), dtype=orig_type)
  uncerts = tf.cast(tf.math.sqrt(tf.stack([dymin, dxmin, dymax, dxmax], axis=-1)), dtype=orig_type) # Revert to std
...
class CalibrateBoxUncert:
  ...
  def __init__(self, model_params, calib_path, general_path=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))):
    ...
    self.calibrators_path = general_path + "/results/calibration/"+calib_path+"/regres_models/"    
    if model_params["calibrate_regression"]:
      iso_calib_percoo, self.iso_calib_all, self.temp_regres_all, temp_regres_per, self.iso_perclscoo, self.iso_perclscoo_rel = self._import_calib_regress()

      self.ymin_calib, self.xmin_calib, self.ymax_calib, self.xmax_calib = [], [], [], []
      if iso_calib_percoo:
        self.ymin_calib, self.xmin_calib, self.ymax_calib, self.xmax_calib = iso_calib_percoo

      self.ymin_calib_temp, self.xmin_calib_temp, self.ymax_calib_temp, self.xmax_calib_temp = [], [], [], []
      if temp_regres_per:
        self.ymin_calib_temp, self.xmin_calib_temp, self.ymax_calib_temp, self.xmax_calib_temp = temp_regres_per
  ...
  def _import_calib_regress(self):
      ...
      if os.path.exists(self.calibrators_path+"regression_calib_iso_perclscoo"): 
        with open(self.calibrators_path+"regression_calib_iso_perclscoo", "rb") as fp: 
          iso_perclscoo = pickle.load(fp)
    
      iso_perclscoo_rel = []
      if os.path.exists(self.calibrators_path+"regression_calib_iso_perclscoo_relative"):
        with open(self.calibrators_path+"regression_calib_iso_perclscoo_relative", "rb") as fp: 
  ...
  def calibrate_boxuncert(self, uncert, classes, boxes):
    ...
    iso_perb = np.array([])
    if self.ymin_calib:
      iso_perb = np.swapaxes([self.ymin_calib.predict(uncert[:,0]), self.xmin_calib.predict(uncert[:,1]), self.ymax_calib.predict(uncert[:,2]), self.xmax_calib.predict(uncert[:,3])],0,1) 

    ts_perb = np.array([])
    if self.ymin_calib_temp:
      ts_perb = np.swapaxes([uncert[:,0]/self.ymin_calib_temp, uncert[:,1]/self.xmin_calib_temp, uncert[:,2]/self.ymax_calib_temp, uncert[:,3]/self.xmax_calib_temp],0,1) 
       
    iso_percl = np.array([])
    if self.iso_perclscoo:
      calibrators = np.asarray(self.iso_perclscoo).reshape(self.model_params["num_classes"], 4)
      iso_percl = np.zeros_like(uncert)
      for ci in range(1, self.model_params["num_classes"]+1):
          if np.any(classes.astype(int)==ci):
            for j in range(4):
                iso_percl[:,j][classes.astype(int)==ci]=(calibrators[ci-1,j].predict(uncert[:,j][classes.astype(int)==ci]))
          else: continue
    ...
    if self.iso_perclscoo_rel:
      ...
      rel_uncert = np.divide(uncert, norm, out=np.zeros_like(uncert), where=norm!=0, dtype=np.float16)
      rel_calibrators = np.asarray(self.iso_perclscoo_rel).reshape(self.model_params["num_classes"], 4)
      rel_calib_uncertpc = np.zeros_like(uncert)
      for ci in range(1, self.model_params["num_classes"]+1):
          if np.any(classes.astype(int)==ci):
            for j in range(4):
                rel_calib_uncertpc[:,j][classes.astype(int)==ci]=(rel_calibrators[ci-1,j].predict(rel_uncert[:,j][classes.astype(int)==ci]))
          else: continue
    ...
    elif self.model_params["calib_method_box"] == "iso_perclscoo" and iso_percl.any():
      select_uncert = iso_percl
    elif self.model_params["calib_method_box"] == "rel_iso_perclscoo" and rel_iso_percl.any():
      select_uncert = rel_iso_percl
    else:
      select_uncert = np.array([])
      print("Unknown calibration method")

    return select_uncert, iso_all, temp_all, ts_perb, iso_perb, iso_percl, rel_iso_percl
