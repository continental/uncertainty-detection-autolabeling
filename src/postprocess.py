      ...
      for i in range(len(uncerts)):
        if uncerts[i] is not None:
          if i!=0: #for regression
            uncerts[i] = tf.gather_nd(uncerts[i], tf.expand_dims(indices, 2), batch_dims=1)
          else: 
...
def pre_nms(params, cls_outputs, box_outputs, topk=True, uncerts=None):
  ...
  if params['loss_attenuation'] and not (params['mc_boxheadrate'] or params['mc_dropoutrate']): uncerts[1] = merge_class_box_level_outputs(params, cls_outputs, uncerts[1])[1]
  if params['mc_classheadrate'] or params['mc_dropoutrate']: uncerts[0] = merge_class_box_level_outputs(params, uncerts[0], box_outputs)[0]
  if params['mc_boxheadrate'] or params['mc_dropoutrate']:   
    ...
    if params['loss_attenuation']:      
      ...
      for j in range(box_outputs[0].shape[0]): # Merge for each feature map
        ...
        al_unc.append(merge_class_box_level_outputs(params, cls_outputs, al_box)[1])
      if batchsizeover1: uncerts[1] = tf.stack(al_unc, axis=0)
      else: uncerts[1] = tf.concat(al_unc, axis=0)
    ...
    if batchsizeover1: box_outputs = tf.stack(boxes, axis=0)
    else: box_outputs = tf.concat(boxes, axis=0)
      ...
      if (params['mc_boxheadrate'] or params['mc_dropoutrate']):  
        if params['loss_attenuation']:     
          ...
          for i in range(params['mc_dropoutsamp']):
            if batchsizeover1: al_unc.append(topk_class_boxes(params, cls_outputs, box_outputs[i], [uncerts[0], uncerts[1][i], uncerts[2]])[-1])
            else: al_unc.append(topk_class_boxes(params, cls_outputs, tf.expand_dims(box_outputs[i],axis=0), [uncerts[0], tf.expand_dims(uncerts[1][i],axis=0), uncerts[2]])[-1])
        ...
        for i in range(params['mc_dropoutsamp']):
          ...
          temp_cls, temp_box, temp_classes, temp_indices, temp_uncerts = topk_class_boxes(params, cls_outputs, temp_box, temp_uncerts)
          boxes.append(temp_box)
  ...
  if params['loss_attenuation'] and not (params['mc_boxheadrate'] or params['mc_dropoutrate']):
      boxes, uncerts[1] = decode_uncert(box_outputs, uncerts[1], anchor_boxes, method=params["uncert_adjust_method"], n_samples=params["decode_nsamples"])
  elif (params['mc_boxheadrate'] or params['mc_dropoutrate']):    
      if params['loss_attenuation']: 
        boxes = []
        aluncdec = []
        for i in range(params['mc_dropoutsamp']):
          temp_box, temp_aluncdec = decode_uncert(box_outputs[i], uncerts[1][i], anchor_boxes, method=params["uncert_adjust_method"], n_samples=params["decode_nsamples"])
          boxes.append(temp_box)
...
def nms(params, boxes, scores, classes, padded, multiclass=None, uncerts1=None, uncerts2=None, uncerts3=None):
  ...
  if uncerts1 is not None: uncerts1 = tf.cast(tf.gather(uncerts1, nms_top_idx), boxes.dtype)
  if uncerts2 is not None: uncerts2 = tf.cast(tf.gather(uncerts2, nms_top_idx), boxes.dtype)
  if uncerts3 is not None: uncerts3 = tf.cast(tf.gather(uncerts3, nms_top_idx), boxes.dtype)
  ...
  if params['loss_attenuation'] or params['mc_dropout']:
    ...
    if params['loss_attenuation'] :
      for i in range(len(box_outputs)):
        ...
        elif params['loss_attenuation'] and (params['mc_boxheadrate'] or params['mc_dropoutrate']):
            split_boxes.append(box_outputs[i][:,:,:,:,:split])
            loss_att_boxes.append(box_outputs[i][:,:,:,:,split:])
...
def postprocess_global(params, cls_outputs, box_outputs, image_scales=None):
  ...
  """
  boxes, uncerts, scores, classes, classes_multi = extract_uncertainties(params, cls_outputs, box_outputs) # None if no softmax
  if params["loss_attenuation"] or params["mc_dropout"]:
  ...
  if classes_multi is not None and uncerts is not None:
    def single_batch_fn(element):
      return nms(params, element[0], element[1], element[2], True, multiclass=element[3], uncerts1=element[4], uncerts2=element[5], uncerts3=element[6])
    nms_boxes, nms_scores, nms_classes, nms_valid_len, nms_class_multi, nms_uncerts0, nms_uncerts1, nms_uncerts2 = batch_map_fn(
      single_batch_fn, [boxes, scores, classes, classes_multi, nms_uncerts[0], nms_uncerts[1], nms_uncerts[2]])

  elif classes_multi is None and uncerts is not None:
    def single_batch_fn(element):
      return nms(params, element[0], element[1], element[2], True, uncerts1=element[4], uncerts2=element[5], uncerts3=element[6])
    nms_boxes, nms_scores, nms_classes, nms_valid_len, nms_uncerts0, nms_uncerts1, nms_uncerts2 = batch_map_fn(
      single_batch_fn, [boxes, scores, classes, nms_uncerts[0], nms_uncerts[1], nms_uncerts[2]])

  elif classes_multi is not None and uncerts is None:
    def single_batch_fn(element):
      return nms(params, element[0], element[1], element[2], True, multiclass=element[3])
    nms_boxes, nms_scores, nms_classes, nms_valid_len, nms_class_multi = batch_map_fn(
      single_batch_fn, [boxes, scores, classes, classes_multi])
  ...
  if uncerts is not None:
    if uncerts[0] is not None: uncerts[0] = nms_uncerts0
    if uncerts[1] is not None: uncerts[1] = nms_uncerts1
    if uncerts[2] is not None: uncerts[2] = nms_uncerts2
...
def per_class_nms(params, boxes, scores, classes, image_scales=None, logits=None):
  ...
  def single_batch_fn(element):
    ...
    boxes_i, scores_i, classes_i, logits = element[0], element[1], element[2], element[3]
    ...
    if logits is not None: 
      nms_class_multi = tf.pad(tf.concat(nms_class_multi, 0), [[0, max_output_size], [0, 0]])
      output.append(tf.gather(nms_class_multi, indices))
    return output
  
  if logits is not None: 
    nms_boxes, nms_scores, nms_classes, nms_valid_len, nms_class_multi = batch_map_fn(
        single_batch_fn, [boxes, scores, classes, logits])
  else:     
    nms_boxes, nms_scores, nms_classes, nms_valid_len = batch_map_fn(
        single_batch_fn, [boxes, scores, classes])
...
def postprocess_per_class(params, cls_outputs, box_outputs, image_scales=None):
  ...
  Returns:
    A tuple of batch level (boxes, scores, classess, valid_len) after nms.
  """
  boxes, _, scores, classes, classes_multi = extract_uncertainties(params, cls_outputs, box_outputs) # None if no softmax
...
def generate_detections(params,
  ...
  if params['enable_softmax']:
    nms_boxes_bs, nms_scores_bs, nms_classes_bs, _, nms_class_multi_bs = postprocess(
    ...
    return generate_detections_from_nms_output(nms_boxes_bs, nms_classes_bs,
                                            ...
                                            original_image_widths, flip, nms_class_multi_bs)
  else:
