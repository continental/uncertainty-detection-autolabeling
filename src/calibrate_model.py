...
from utils_extra import gt_box_assigner, update_arrays
...
class Calibrate:
    ...
    def __init__(
        ...
        if model_params["enable_softmax"]:
            ...
            if model_params["mc_classheadrate"] or model_params["mc_dropoutrate"]:
                self.filtered_classes_mccls = np.array([])
            else:
                self.filtered_classes_mccls = None

        if model_params["loss_attenuation"]:
            self.filtered_uncert_albox = np.array([])
        else:
            self.filtered_uncert_albox = None

        if model_params["mc_boxheadrate"] or model_params["mc_dropoutrate"]:
            self.filtered_uncert_mcbox = np.array([])
        else:
            self.filtered_uncert_mcbox = None
    ...
    def gather_detections(self):
        ...
        for im_name in self.img_names:
            ...
            image_file = tf.io.read_file(
                self.save_dir.split("models")[0] + "/datasets/" + image_npath
            )
            im_name = tf.io.decode_image(
            ...
            if self.model_params["enable_softmax"]:
                box, scores, classe, _, logits = tf.nest.map_structure(
            ...
            for i in range(box[0].shape[0]):
                ...
                if self.gt_classes[ind][i] >= 0:
                    self.filtered_gt_classes = np.append(
                        self.filtered_gt_classes, self.gt_classes[ind][i] - 1
                    )
                    self.filtered_gt_coords = update_arrays(
                        self.filtered_gt_coords, self.gt_coords[ind], i
                    )
                    correct_index = gt_box_assigner(
                        self.model_params["assign_gt_box"],
                        self.gt_coords[ind],
                        box[0],
                        i,
                    )
                    ...
                    self.filtered_classes = np.append(
                        self.filtered_classes, classe[0][correct_index]
                    )
                    self.filtered_boxes = update_arrays(
                        self.filtered_boxes, box[0], correct_index
                    )

                    if self.model_params["calibrate_regression"]:
                        if self.model_params["loss_attenuation"]:
                            self.filtered_uncert_albox = update_arrays(
                                self.filtered_uncert_albox, al_boxhead[0], correct_index
                            )
                        if (
                            self.model_params["mc_boxheadrate"]
                            or self.model_params["mc_dropoutrate"]
                        ):
                            self.filtered_uncert_mcbox = update_arrays(
                                self.filtered_uncert_mcbox, mc_boxhead[0], correct_index
                            )
                    if (
                        self.model_params["calibrate_classification"]
                        and self.model_params["enable_softmax"]
                    ):
                        ...
                        if (
                            ...
                            self.filtered_classes_mccls = update_arrays(
                                self.filtered_classes_mccls,
                                mc_classhead[0],
                                correct_index,
                            )
            ind += 1
    ...
    def calibrate_regclas(self):
        ...
        if self.filtered_classes_mccls is not None:
            self.filtered_classes_mccls = self.filtered_classes_mccls[filterd]
        if self.filtered_uncert_albox is not None:
            self.filtered_uncert_albox = self.filtered_uncert_albox[filterd]
        if self.filtered_uncert_mcbox is not None:
            self.filtered_uncert_mcbox = self.filtered_uncert_mcbox[filterd]

        if (
            self.model_params["calibrate_classification"]
            and self.model_params["enable_softmax"]
            and self.model_params["num_classes"] > 1
        ):
            os.makedirs(
                self.saving_path + self.save_dir.split("/")[-1] + "/classification/"
            )
            ClassificationCalib(
        ...
        if self.model_params["calibrate_regression"]:
            if self.filtered_uncert_albox is not None:
                os.makedirs(
                    self.saving_path
                    + self.save_dir.split("/")[-1]
                    + "/regression/aleatoric/"
                )
                RegressionCalib(
                    self.filtered_gt_coords,
                    self.filtered_boxes,
                    np.nan_to_num(self.filtered_uncert_albox),
                    self.filtered_gt_classes,
                    self.save_dir.split("/")[-1] + "/regression/aleatoric/",
                    self.saving_path,
                ).box_uncert_calibration()
            if self.filtered_uncert_mcbox is not None:
                os.makedirs(
                    self.saving_path
                    + self.save_dir.split("/")[-1]
                ...
                RegressionCalib(
                    self.filtered_gt_coords,
                    self.filtered_boxes,
                    np.nan_to_num(self.filtered_uncert_mcbox),
                    self.filtered_gt_classes,
                    self.save_dir.split("/")[-1] + "/regression/mcdropout/",
                    self.saving_path,
                ).box_uncert_calibration()
