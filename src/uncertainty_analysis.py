...
CALIB_METHOD_CLS = default_detection_configs().calib_method_class
CALIB_METHOD_BOX = default_detection_configs().calib_method_box
...
def roc_metrics(uncert, y_true):
    ...
    if FIX_CD:
        if all(fpr > 1 - FPR_TPR):
            ...
            return (
                min(map(lambda idx: thresholds[idx], idxs)),
                min(map(lambda idx: (1 - tpr)[idx], idxs)),
                roc_auc,
            )
        else:
        ...
        if all(tpr < FPR_TPR):
            ...
            return (
                min(map(lambda idx: thresholds[idx], idxs)),
                min(map(lambda idx: fpr[idx], idxs)),
                roc_auc,
            )
        else:
    ...
    def _extract_optimal_params(self, break_iter=10):
        ...
        def _f_x(params):
            ...
            for iou_thr in IOU_THRS:
                correct_detections = np.asarray(
                    (self.ious >= iou_thr) * self.tps_class, dtype=int
                )
                if self.per_cls:
                    ...
                    for i in range(self.num_classes):
                        for j in range(len(self.uncert)):
                            collected_uncert[j][self.gt_classes == i + 1] *= params[
                                n_iter
                            ]  # Important test with pred
                            n_iter += 1
                    uncert = np.sum(collected_uncert, axis=0)
                else:
                    uncert = sum(
                        param * uncert for param, uncert in zip(params, self.uncert)
                    )
        ...
        if self.per_cls:
            ...
            num_params = len(self.uncert)

        if self.method == "hebo":
            space = DesignSpace().parse(
                [
                    {"name": f"param_{i}", "type": "num", "lb": 0, "ub": 1}
            ...
            for i in range(300):
                ...
                if min_f == prev_min_f:
                    ...
                    if num_iter_since_change == break_iter:
                        print(
                            "Minimum objective value has not changed for 10 iterations. Breaking early."
                        )
                        break
                else:
                    prev_min_f = min_f
            ...
            self.opt_params = [
                study.best_params[f"param_{i}"] for i in range(1, num_params + 1)
            ]
        ...
        with open(
            ...
            file.write(str(np.asarray(self.opt_params, dtype="object")))
        ...
        for iou_thr in IOU_THRS:
            ...
            if self.per_cls:
                ...
                uncert = sum(
                    param * uncert
                    for param, uncert in zip(self.opt_params, self.uncert)
                )
...
class MainUncertViz:
    ...
    def __init__(
        ...
        if self.albox is not None:
            self.albox = relativize_uncert(self.pred_boxes, self.albox)
        if self.mcbox is not None:
            self.mcbox = relativize_uncert(self.pred_boxes, self.mcbox)
        if self.calib_albox is not None:
            self.calib_albox = relativize_uncert(self.pred_boxes, self.calib_albox)
        ...
        all_opt_params = UncertOptimal(
            ...
            added_name="",
        ).get_optimal_uncertainty()
        self.opt_uncert = sum(
            opt_param * uncert
            for opt_param, uncert in zip(all_opt_params, self.selected_uncerts)
        )
        self._save_thr_performance(added_name="", opt_params=all_opt_params)

        if self.per_cls:
            perc_opt_params = UncertOptimal(
                self.gt_classes,
                self.tps_class,
                self.ious,
                self.selected_uncerts,
                ...
                added_name="_clsopt",
                per_cls=True,
            ).get_optimal_uncertainty(break_iter=30)
            # Pred
            ...
            for i in range(num_classes):
                per_cls_uncert = [
                    collected_uncert[j][self.gt_classes == i + 1]
                    for j in range(len(self.selected_uncerts))
                ]
                opt_uncert = sum(
                    opt_param * uncert
                    for opt_param, uncert in zip(all_opt_params, per_cls_uncert)
                )
                if len(opt_uncert) > 0:
                    all_fdcd = self._save_thr_performance(
                        selected_uncert=per_cls_uncert,
                        opt_uncert=opt_uncert,
                        tps_class=self.tps_class[self.gt_classes == i + 1],
                        ious=self.ious[self.gt_classes == i + 1],
                    ...
                    opt_uncert = sum(
                        opt_param * uncert
                        for opt_param, uncert in zip(temp_opt_params, per_cls_uncert)
                    )
                    current_fdcd = self._save_thr_performance(
                        selected_uncert=per_cls_uncert,
                        opt_uncert=opt_uncert,
                        tps_class=self.tps_class[self.gt_classes == i + 1],
                        ious=self.ious[self.gt_classes == i + 1],
                    ...
                    if (
                        current_fdcd < all_fdcd or current_fdcd == 100
                    ):  # If worse than for all
                        temp_n_iter = n_iter
            ...
            self._save_thr_performance(
                opt_uncert=fixed_opt_uncert,
                added_name="_clsoptfix",
                opt_params=perc_opt_params,
            )
    ...
    def _extract_det(detections, ious, value):
        ...
        return np.asarray([detections[i][value] for i in range(len(detections))])[
            ious > 0.0
        ]

    def _save_thr_performance(
        self,
        selected_uncert=None,
        opt_uncert=None,
        tps_class=None,
        ious=None,
        added_name="",
        ...
        if selected_uncert is None:
            selected_uncert = self.selected_uncerts
        if opt_uncert is None:
            opt_uncert = self.opt_uncert
        if tps_class is None:
            tps_class = self.tps_class
        if ious is None:
            ious = self.ious
        ...
        def _metrics_iou(uncert, tps_class=tps_class, ious=ious):
            ...
            for iou_thr in IOU_THRS:
                ...
                jsds.append(
                    np.nan_to_num(
                        calc_jsd(
                            uncert[correct_detections], uncert[~correct_detections]
                        )
                    )
                )
        ...
        jsds_orig, aucs_orig, fprs_orig = zip(
            *[_metrics_iou(uncert) for uncert in selected_uncert]
        )
        ...
        if opt_params is not None:
            ...
            for i in range(num_classes):
                ...
                if len(selected_inds) > 0:
                    ...
                    selected_uncert_class = [
                        unc[selected_inds] for unc in selected_uncert
                    ]
                    if len(opt_params) == len(selected_uncert):
                        opt_uncert_class = sum(
                            opt_param * uncert
                            for opt_param, uncert in zip(
                                opt_params, selected_uncert_class
                            )
                        )
                    else:
                    ...
                    jsds_class, aucs_class, fprs_class = _metrics_iou(
                        opt_uncert_class, selected_tpclass, selected_ious
                    )
                else:
                    jsds_class = aucs_class = fprs_class = [0] * len(IOU_THRS)
            ...
            for i in range(num_classes):
                table_data.append(
                    [self.label_map[i + 1]]
                    + list(np.round(class_aucs[i], 2))
                    + [np.round(np.mean(class_aucs[i]), 2)]
                    + list(np.round(class_jsds[i], 2))
                    + [np.round(np.mean(class_jsds[i]), 2)]
                    + list(np.round(class_fprs[i], 2))
                    + [np.round(np.mean(class_fprs[i]), 2)]
                )
            avg_row = []
            for i in range(1, len(table_data[0])):
                avg_row.append(
                    np.round(
                        np.mean((np.asarray(table_data)[:, i][2:]).astype("float")), 2
                    )
                )
            table_data.append(["Avg. perc"] + avg_row)

            for i in range(len(aucs_orig)):
                table_data.append(
                    ["Orig. U." + str(i)]
                    + list(np.round(aucs_orig[i], 2))
                    + [np.round(np.mean(aucs_orig[i]), 2)]
                    + list(np.round(jsds_orig[i], 2))
                    + [np.round(np.mean(jsds_orig[i]), 2)]
                    + list(np.round(fprs_orig[i], 2))
                    + [np.round(np.mean(fprs_orig[i]), 2)]
                )
            table_data.append(
                ["Opt. Combo"]
                + list(np.round(aucs_opt, 2))
                + [np.round(np.mean(aucs_opt), 2)]
                + list(np.round(jsds_opt, 2))
                + [np.round(np.mean(jsds_opt), 2)]
                + list(np.round(fprs_opt, 2))
                + [np.round(np.mean(fprs_opt), 2)]
            )
        ...
        with open(
            ...
            for idx, (jsd, auc, fpr) in enumerate(
                zip(jsds_orig, aucs_orig, fprs_orig), start=1
            ):
                file.write(
                    f"Uncertainty {idx}: JSD = {jsd:.3f}, AUC = {auc:.3f}, {metric} = {fpr:.3f}\n"
                )

            file.write("\nJSD, AUC, " + metric + " of Optimal Uncertainty:\n")
            file.write(
                f"JSD = {jsds_opt:.3f}, AUC = {aucs_opt:.3f}, {metric} = {fprs_opt:.3f}\n"
            )
    ...
    def _collect_thresh_results(self, eval_iou_thr):
        ...
        correct_detections = np.asarray(
            (self.ious >= eval_iou_thr) * self.tps_class, dtype=int
        )
        opt_thr = roc_metrics(self.opt_uncert, correct_detections)[0]
        correctly_removed = (self.opt_uncert >= opt_thr) * ~correct_detections.astype(
            "bool"
        )
        falsely_removed = (self.opt_uncert >= opt_thr) * correct_detections.astype(
            "bool"
        )
        falsely_remaining = (self.opt_uncert < opt_thr) * ~correct_detections.astype(
            "bool"
        )
    ...
    def _draw_postthresholding(self, img_name, title):
        ...
        im = visualize_image(
            img_array,
            self.pred_boxes[det_ind],
            self.pred_classes.astype("int")[det_ind],
            self.pred_scores[det_ind],
            self.label_map,
            min_score_thresh=0.0001,
        )
        gt_im = visualize_image(
            img_array,
            self.gt_boxes[det_ind],
            self.gt_classes.astype("int")[det_ind],
            np.ones_like(self.pred_scores[det_ind]),
            self.label_map,
            min_score_thresh=0.0001,
        )
        ...
        for label_key, color_value in labels.items():
            ...
            for i in det_ind[values[label_key][det_ind]]:
                x, y, w, h = [
                    self.pred_boxes[i][1],
                    self.pred_boxes[i][0],
                    self.pred_boxes[i][3] - self.pred_boxes[i][1],
                    self.pred_boxes[i][2] - self.pred_boxes[i][0],
                ]
    ...
    def _collect_postthresholding(self):
        ...
        correctly_removed, falsely_removed, falsely_remaining, opt_thr = (
            self.pred_filters
        )
        ...
        inds = np.asarray(
            [
                np.all(self.opt_uncert[indices_dict[iname]] < opt_thr)
                for iname in unique_images
            ]
        )
        filtered_imgs = unique_images[inds]
        random_indexes = random.sample(range(len(filtered_imgs)), 10)
        [
            self._draw_postthresholding(im_name, folders_names[3])
            for im_name in filtered_imgs[random_indexes]
        ]
    ...
    def _read_predictions(self):
        ...
        self.gt_boxes = np.asarray(
            [detections[i]["gt_bbox"] for i in range(len(detections))]
        )
        self.pred_boxes = np.asarray(
            [detections[i]["bbox"] for i in range(len(detections))]
        )
        ...
        if "entropy" in detections[0]:
            self.entropy = self._extract_det(detections, self.ious, "entropy")
        if CALIB_METHOD_CLS + "_entropy" in detections[0]:
            self.calib_entropy = self._extract_det(
                detections, self.ious, CALIB_METHOD_CLS + "_entropy"
            )
        if CALIB_METHOD_BOX + "_albox" in detections[0]:
            self.calib_albox = self._extract_det(
                detections, self.ious, CALIB_METHOD_BOX + "_albox"
            )
        if "uncalib_albox" in detections[0]:
            self.albox = self._extract_det(detections, self.ious, "uncalib_albox")
        if "uncalib_mcbox" in detections[0]:
            self.mcbox = self._extract_det(detections, self.ious, "uncalib_mcbox")
        if "uncalib_mcclass" in detections[0]:
            self.mcclass = self._extract_det(detections, self.ious, "uncalib_mcclass")
    ...
    def _plot_validheat(self):
        ...
        def _heatmap_plot(pred, value, plot_title, save_title, uncert=True):
            ...
            for bb in pred:
                ...
                i += 1
            if uncert:
                mask = np.nan_to_num(mask / normalizer)
            ax = plt.gca()
        ...
        _heatmap_plot(
            self.gt_boxes,
            np.mean(np.ones_like(self.gt_boxes), axis=1),
            "Location Frequency",
            "gt_loc.png",
            uncert=False,
        )
        ...
        if self.albox is not None:
            _heatmap_plot(
                self.pred_boxes,
                np.mean(self.albox, axis=1),
                "Norm. Mean Aleatoric Uncertainty",
                "uncert_albox.png",
            )
            rmse = np.sqrt(
                np.mean(
                    (np.abs(self.gt_boxes - self.pred_boxes) - self.albox) ** 2, axis=-1
                )
            )
            _heatmap_plot(
                self.gt_boxes,
                rmse,
                "RMSE Residuals/Aleatoric Uncertainty",
                "rmse_albox.png",
            )
        ...
        if self.calib_albox is not None:
            _heatmap_plot(
                self.pred_boxes,
                np.mean(self.calib_albox, axis=1),
                "Norm. Mean Aleatoric Uncertainty",
                "uncert_calib_albox.png",
            )
            rmse = np.sqrt(
                np.mean(
                    (np.abs(self.gt_boxes - self.pred_boxes) - self.calib_albox) ** 2,
                    axis=-1,
                )
            )
            _heatmap_plot(
                ...
                rmse,
                "RMSE Residuals/Aleatoric Uncertainty",
                "rmse_calib_albox.png",
            )

        if self.entropy is not None:
            _heatmap_plot(self.pred_boxes, self.entropy, "Entropy", "entropy.png")

        if self.calib_entropy is not None:
            _heatmap_plot(
                self.pred_boxes, self.calib_entropy, "Entropy", "calib_entropy.png"
            )

        _heatmap_plot(
            self.pred_boxes[self.pred_filters[0]],
            self.opt_uncert[self.pred_filters[0]],
            "Optimal Uncertainty Correctly Removed",
            "opt_uncert_cr.png",
        )
        _heatmap_plot(
            self.pred_boxes[self.pred_filters[1]],
            self.opt_uncert[self.pred_filters[1]],
            "Optimal Uncertainty Falsely Removed",
            "opt_uncert_fr.png",
        )
        _heatmap_plot(
            self.pred_boxes[self.pred_filters[2]],
            self.opt_uncert[self.pred_filters[2]],
            "Optimal Uncertainty Falsely Remaining",
            "opt_uncert_fk.png",
        )
    ...
    def _plot_metricsspider(self):
        ...
        for iou_thr in IOU_THRS:
            ...
            metrics_all_data.append(
                [np.mean(self.ious), np.mean(self.tps_class), 1, 1, 1]
            )
            metrics_removed_data.append(
                [
                    np.mean(self.ious[self.opt_uncert >= thr]),
                    np.mean(self.tps_class[self.opt_uncert >= thr]),
                    len(correct_detections[self.opt_uncert >= thr])
                    / len(correct_detections),
                    np.sum(correct_detections[self.opt_uncert >= thr])
                    / np.sum(correct_detections),
                    np.sum(1 - correct_detections[self.opt_uncert >= thr])
                    / np.sum(1 - correct_detections),
                ]
            )

            metrics_remaining_data.append(
                [
                    np.mean(self.ious[self.opt_uncert < thr]),
                    np.mean(self.tps_class[self.opt_uncert < thr]),
                    len(correct_detections[self.opt_uncert < thr])
        ...
        collected_data = [
            np.mean(metrics_all_data, axis=0) * 100,
            np.mean(metrics_removed_data, axis=0) * 100,
            np.mean(metrics_remaining_data, axis=0) * 100,
        ]
        ...
        for _, (det, label, color) in enumerate(zip(collected_data, labels, colors)):
            ...
            ax.plot(
                angles, det, color=color, linestyle="solid", linewidth=1, label=label
            )
        ...
        plt.savefig(
            self.source_path + "/thr_plots/spider_metrics.png", bbox_inches="tight"
        )
        plt.close()
