...
class ServingDriver:
    ...
    def visualize(self, image, boxes, classes, scores, uncertainty=None, **kwargs):
    ...
    def _benchmark(self, image_arrays, test_func, bm_runs=10, trace_filename=None):
    ...
    def _preprocess(self, image_arrays):
        ...
        def map_fn(image):
            ...
            input_processor.normalize_image(
                self.params["mean_rgb"], self.params["stddev_rgb"]
            )
    ...
    def _postprocess(self, outputs, scales):
        det_outputs = postprocess.postprocess_global(
            self.params, outputs[0], outputs[1], scales
        )
...
class KerasDriver(ServingDriver):
    ...
    def predict(self, image_arrays):
        if self.only_network:
            outputs = tuple(
                self.model(image_arrays, pre_mode=None, post_mode=None)
    ...
    def export(
        ...
        if output_dir:
            tf.saved_model.save(
                ...
                output_dir,
                signatures={
                    tf.saved_model.DEFAULT_SERVING_SIGNATURE_DEF_KEY: export_model.__call__.get_concrete_function(
                        input_spec
                    ),
                    "predict": export_model.predict.get_concrete_function(
                        tflite_input_spec
                    ),
                },
