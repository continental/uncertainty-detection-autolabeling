...
def define_flags():
    ...
    flags.DEFINE_string(
        ...
        default=None,
        help="Specify which GPU(s) to use. This sets the CUDA_VISIBLE_DEVICES "
...
def set_gpu_configuration():
    """Set the GPU configuration based on the GPU FLAG."""
...
def main(_):
    ...
    with ds_strategy.scope():
        ...
        activate_aug = False
        if "imscore" in FLAGS.train_file_pattern:
            activate_im_score = True
            logging.info("Imscore activated")
        if "aug" in FLAGS.train_file_pattern:
        ...
        if config.count_classes:
            Count_class_instances(train_dataset, val_dataset, config).count_dataset(
                FLAGS.num_examples_per_epoch, True
            )
        print(config)
        ...
        for epoch in range(initial_epoch, FLAGS.num_epochs):
            ...
            for step, batch in enumerate(train_dataset):
                # Check if the batch is a tuple (zipped dataset) or not (single dataset)
                if isinstance(batch[1], dict):
                    x_batch_train, y_batch_train = batch
                else:
                    x_batches = [x for (x, _) in batch]
...
if __name__ == "__main__":
    define_flags()
    FLAGS(sys.argv)  # Parse flags here
    set_gpu_configuration()  # Set GPU config after parsing flags
