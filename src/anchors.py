...
import utils
from object_detection_efficientdet import argmax_matcher
from object_detection_efficientdet import box_list
from object_detection_efficientdet import faster_rcnn_box_coder
from object_detection_efficientdet import region_similarity_calculator
from object_detection_efficientdet import target_assigner
...
def decode_box_outputs(pred_boxes, anchor_boxes):
  ...
  ymax = ycenter + h / 2.
  xmax = xcenter + w / 2.
  coords = tf.stack([ymin, xmin, ymax, xmax], axis=-1)
  # measure method runtime
  # elapsed_time = time.time() - st
  # txt_path = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + '/misc/misc_plots/decoding_runtime_orig.txt'
  ...
  def _generate_boxes(self):
    ...
    for _, configs in self.config.items():
      ...
      for config in configs:
        ...
        # print("%--------------------Saving anchors in anchors.py generate_boxes-------------------")
        # plt.plot(xv, yv, ".k")
        # # bottom left corner , width, height
        # for bbb in boxes:
        #     rect=mpatches.Rectangle((bbb[1], bbb[0]),bbb[3]-bbb[1],bbb[2]-bbb[0],
        #                     fill = False,
        #                     color = "purple",
        #                     linewidth = 1)
      ...
      # j+=1
      # plt.savefig(' os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + '/misc/misc_plots/meshgrid_'+str(j)+'_'+str(i)+'.png')
      # print("%--------------------Saving anchors in anchors.py generate_boxes-------------------")
