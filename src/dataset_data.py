...
def get_ocl_trc(model_dir, img_names):
    ...
    if "KITTI" in model_dir:
        for im_name in img_names:
            ...
            if occl:
                occlusions.append(occl)
            else:
                occlusions.append([-1] * 100)
            if trcs:
                truncations.append(trcs)
            else:
        ...
        f = json.load(
            open(
                model_dir.split("models")[0]
                + "/datasets/BDD100K/bdd100k/labels/bdd100k_labels_images_val.json"
            )
        )
        for j in range(len(f)):
            occl = [
                f[j]["labels"][i]["attributes"]["occluded"]
                for i in range(len(f[j]["labels"]))
                if f[j]["labels"][i]["category"] in labels
            ]
            trcs = [
                f[j]["labels"][i]["attributes"]["truncated"]
                for i in range(len(f[j]["labels"]))
                if f[j]["labels"][i]["category"] in labels
            ]
            if occl:
