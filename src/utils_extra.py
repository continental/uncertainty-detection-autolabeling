...
from utils_box import relativize_uncert, calc_iou_np, calc_rmse, calc_ece
...
def calc_jsd(dist_1, dist_2):
    ...
    x = np.linspace(min(mean_1 - 3 * std_1, mean_2 - 3 * std_2), max(mean_1 + 3 * std_1, mean_2 + 3 * std_2), 1000)
...
def gt_box_assigner(sorting_method, gt_box, boxes, i):
  ...
  if sorting_method == "MSE": 
    correct_index = np.argmin(np.mean(np.square([gt_box[i]] * len(boxes) - boxes), axis=1))
...
def save_uncert(path, uncert, title):
  ...
  with open(path+'/uncert'+title+'.txt', 'a') as f:
    if len(uncert) != 0: uncert = np.max(uncert) # Per image
    else: uncert = np.NaN
    f.write(str(uncert)+"\n")
...
def mc_eval(mc_model, images, config):
  ...
  Returns:
      Stacked output
  """
  if config.mc_classheadrate or config.mc_dropoutrate: cls_outputs0, cls_outputs1, cls_outputs2, cls_outputs3, cls_outputs4 = [], [], [], [], [] 
  if config.mc_boxheadrate or config.mc_dropoutrate: box_outputs0, box_outputs1, box_outputs2, box_outputs3, box_outputs4 = [], [], [], [], []
  output = []
  ...
  if config.mc_classheadrate or config.mc_dropoutrate: cls_concat = stack_mcpred([cls_outputs0, cls_outputs1, cls_outputs2, cls_outputs3, cls_outputs4])    
  else: cls_concat = cls # Deterministic       
  if config.mc_boxheadrate or config.mc_dropoutrate: box_concat = stack_mcpred([box_outputs0, box_outputs1, box_outputs2, box_outputs3, box_outputs4])  
  else: box_concat = box       
...
def get_mcuncert(output):
  ...
  """
  output0, output1, output2, output3, output4 = output 
  mean = [tf.reduce_mean(output0, axis=0),tf.reduce_mean(output1, axis=0),tf.reduce_mean(output2, axis=0),tf.reduce_mean(output3, axis=0),tf.reduce_mean(output4, axis=0)]   
  uncert = [tf.math.reduce_std(output0, axis=0),tf.math.reduce_std(output1, axis=0),tf.math.reduce_std(output2, axis=0),tf.math.reduce_std(output3, axis=0),tf.math.reduce_std(output4, axis=0)] 
  return mean, uncert
...
def bin_values(x, y, n_bins):
  ...
  mean= np.asarray([np.mean(y[[bins[i-1]<x][0]*[x<=bins[i]][0]] if len(y[[bins[i-1]<x][0]*[x<=bins[i]][0]])!=0 else 0) for i in range(0,len(bins))])
  std= np.asarray([np.std(y[[bins[i-1]<x][0]*[x<=bins[i]][0]] if len(y[[bins[i-1]<x][0]*[x<=bins[i]][0]])!=0 else 0) for i in range(0,len(bins))])
  count = np.asarray([len(np.where([bins[i-1]<x][0]*[x<=bins[i]][0]==True)[0]) for i in range(0,len(bins))])
  centers = np.asarray([0] + [(bins[i]+bins[i-1])/2 for i in range(1,len(bins))])
...
def mtplt_to_img(figure):
    ...
    plt.close(figure)
    buf.seek(0)
    image = tf.image.decode_png(buf.getvalue(), channels=4) # Convert to tensor png image
...
def plot_conf_matrix(conf_matrix, class_names):
  ...
  for i, j in itertools.product(range(conf_matrix.shape[0]), range(conf_matrix.shape[1])):
      color = "white" if conf_matrix[i, j] > threshold else "black"  # Use white text if squares are dark; otherwise black
...
class ValidUncertPlot:
  ...
  def __init__(self, gt_boxes, pred_boxes, gt_classes, pred_classes, uncert, calib_uncert, save_path, model_params):
    """ 
    Args:
    ...
    if os.path.exists(os.path.dirname(self.save_path)+"/model_performance.txt"):
      with open(os.path.dirname(self.save_path)+"/model_performance.txt", "a") as f: 
        f.write("ECE "+self.save_path.split("/")[-1]+": " + str(ece)+ " \n") 
    else:
      with open(os.path.dirname(self.save_path)+"/model_performance.txt", "a") as f: 
        ...
        f.write("Uncertainty Decoding: " + model_params["uncert_adjust_method"]+ " \n")
        f.write("ECE Loc "+self.save_path.split("/")[-1]+": " + str(ece)+ " \n") 
    # Uncertainty quality
    self.collect_uvi()
  ...
  def _uvi_plot_metrics(self, gt_boxes, pred_boxes, uncert, title="uncalibrated_all"):
    ...
    ax1 = uvi.plot_xy(pred_boxes[0:100], uncert[0:100], gt_boxes[0:100], np.arange(len(gt_boxes[0:100])), ax=ax1) 
    ...
    plt.savefig(self.save_path+"/uncertainty_toolbox/uvi_"+title+".png", bbox_inches='tight') 
    plt.close()

    metrics = uct.metrics.get_all_metrics(pred_boxes.astype(np.float32), uncert.astype(np.float32), gt_boxes.astype(np.float32))
    with open(self.save_path+"/uncertainty_toolbox/uvi_"+title+".txt", "w") as file: 
  ...
  def _cdf(self):
    ...
    pcdf = np.zeros(n)
    for i in range(n):
        pcdf[i] = scipy.stats.norm.cdf(self.gt_boxes.flatten()[i], loc=self.pred_boxes.flatten()[i], scale = self.uncert.flatten()[i])
    return pcdf
  ...
  def collect_uvi(self):
    ...
    def _plot_cdf(cdf, step=0.01):
        ...
        for i in range(len(x)): cumulative_cdf[i] = np.mean(cdf <= x[i])
        fig, ax = plt.subplots()
    ...
    _plot_cdf(self._cdf())
    self._uvi_plot_metrics(self.gt_boxes.flatten(), self.pred_boxes.flatten(), self.uncert.flatten())    
    if self.calib_uncert is not None: self._uvi_plot_metrics(self.gt_boxes.flatten(), self.pred_boxes.flatten(), self.uncert.flatten(), "calibrated_all")
    
    coords_names = ["ymin","xmin","ymax","xmax"]
    for i in range(4):
      self._uvi_plot_metrics(self.gt_boxes[:,i], self.pred_boxes[:,i], self.uncert[:,i], "uncalibrated_"+coords_names[i])    
      if self.calib_uncert is not None: self._uvi_plot_metrics(self.gt_boxes[:,i], self.pred_boxes[:,i], self.uncert[:,i], "calibrated_"+coords_names[i]) 
