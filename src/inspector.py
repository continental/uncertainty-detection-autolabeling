...
def main(mode, dataset_name="k", general_path=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))):
  ...
  if len(dataset_name) > 5: yaml_path = general_path+'/configs/inference/AL/inference_'+dataset_name+'.yaml'
  else: yaml_path = general_path+'/configs/inference/inference_'+dataset_name+'.yaml'
  ...
  video_save_path = general_path+"/results/inference/videos/"+saved_model_dir.split("/")[-1]  
  ...
  if mode == 'calibrate' or mode == 'validate':
    ...
    val_dataset, img_names = dataloader.InputReader(
            ...
            debug=False)(model_config.as_dict(), names=True)
    ...
    for (_, labels) in val_dataset:
        ...
        gt_classes = np.vstack((gt_classes, labels['groundtruth_data'][0][:,-1])) if gt_classes.size > 0 else np.array([labels['groundtruth_data'][0][:,-1]])
        gt_coords = np.vstack((gt_coords, [labels['groundtruth_data'][0][:,0:4]])) if gt_coords.size > 0 else np.array([labels['groundtruth_data'][0][:,0:4]])    
    driver = infer_lib.ServingDriver.create(
    model_dir, debug, saved_model_dir, model_name,
    batch_size or None, only_network, model_params)

  if mode == 'export':
    print("Exporting model") 
    driver = infer_lib.KerasDriver(model_dir, debug,
                                   model_name, batch_size or None,
                                   only_network, model_params)
    ...
    driver.export(saved_model_dir, tensorrt, tflite, file_pattern,
    ...
    driver = infer_lib.ServingDriver.create(
        model_dir, debug, saved_model_dir, model_name,
        batch_size or None, only_network, model_params)
    InferImages(infer_data, driver, model_params, saved_model_dir, mode, general_path).iterate_infer()

  elif mode == 'calibrate':
    print("Calibrating model")   
    Calibrate(model_params, img_names, driver, gt_classes, gt_coords, saved_model_dir, general_path).calibrate_regclas()

  elif mode == 'validate':
    print("Validating model")    
    Validate(model_params, img_names, driver, gt_classes, gt_coords, saved_model_dir, general_path).launch_val()
    ...
    while cap.isOpened():
      ...
      if len(scores[0][scores[0]>min_score]) !=0:
        ...
        if out_ptr: # Save whole video
          ...
          import sys, select
          print("Press anything to stop saving frames. You have 2 seconds to answer!")
          inpt, _, _ = select.select( [sys.stdin], [], [], 2)
          if inpt:
            break
          else:
            imgs_list.append(video_save_path+"/frame_"+str(i)+".png")
            cv2.imwrite(video_save_path+"/frame_"+str(i)+".png", new_frame)
            i+=1            
...
if __name__ == "__main__":
    ...
    modes = ["export", "inference", "calibrate", "validate", "video", "auto-label", "SSAL"]
    ...
    parser.add_argument("--mode", type=int, choices=range(len(modes)), help="Select a mode")
    parser.add_argument("--general_path", type=str, help="Path to workspace", default=os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    ...
    if args.dataset is not None and args.mode is not None:
        ...
        try:  
          dataset_choice = str(input("Enter the dataset letter: "))
