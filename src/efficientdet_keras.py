...
class FNode(tf.keras.layers.Layer):
    ...
    def fuse_features(self, nodes):
        ...
        if self.weight_method == "attn":
            ...
            edge_weights = [tf.nn.relu(tf.cast(var, dtype=dtype)) for var in self.vars]
    ...
    def _conv_bn_act(self, image, i, level_id, training):
        ...
        def _call(image):
            ...
            if self.act_type:
                ...
                if self.dropoutrate:
                    image = tf.keras.layers.SpatialDropout2D(self.dropoutrate)(
                        image, training=True
                    )  # Add mc dropout
...
class EfficientDetNet(tf.keras.Model):
    ...
    def __init__(self, model_name=None, config=None, name="", feature_only=False):
        """Initialize model."""
        super().__init__(name=name)
        ...
        for head in config.heads:
            if head == "object_detection":
                if config.mc_dropout:  # Activate mc dropout
                    self.mc_dropoutboxrate = (
                        config.mc_boxheadrate or config.mc_dropoutrate
                    )
                    self.mc_dropoutclassrate = (
                        config.mc_classheadrate or config.mc_dropoutrate
                    )
                else:
                ...
                if config.loss_attenuation:
                    extend_output = (
                        2 * num_anchors
                    )  # instead of 4*n_anchors => 8*n_anchors
                else:
    ...
    def call(self, inputs, training):
        ...
        if config.mc_dropout and not config.is_training_bn:
            ...
            if config.mc_classheadrate or config.mc_dropoutrate:
                ...
                cls_concat = cls  # Should be deterministic
            if config.mc_boxheadrate or config.mc_dropoutrate:
                ...
                box_concat = box  # Should be deterministic
...
class EfficientDetModel(EfficientDetNet):
    ...
    def _preprocessing(self, raw_images, image_size, mean_rgb, stddev_rgb, mode=None):
    ...
    def call(self, inputs, training=False, pre_mode="infer", post_mode="global"):
        ...
        # preprocess.
        inputs, scales = self._preprocessing(
            inputs, config.image_size, config.mean_rgb, config.stddev_rgb, pre_mode
        )
        # network.
