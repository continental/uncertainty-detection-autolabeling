...
import postprocess
import utils_keras
from utils_extra import stack_mcpred
  ...
  def _conv_bn_act(self, image, i, level_id, training):
    ...
    def _call(image):
      ...
      if self.act_type:
        ...
        if self.dropoutrate: image = tf.keras.layers.SpatialDropout2D(self.dropoutrate)(image, training=True)  # Add mc dropout
...
class EfficientDetNet(tf.keras.Model):
  ...
  def __init__(self,
    ...
    if 'efficientnet' in backbone_name:
      ...
      self.backbone = backbone_factory.get_model(
          backbone_name, override_params=override_params, extra_config=config)
    ...
    for head in config.heads:
      if head == 'object_detection':
        if config.mc_dropout: # Activate mc dropout
          self.mc_dropoutboxrate = config.mc_boxheadrate or config.mc_dropoutrate
          self.mc_dropoutclassrate = config.mc_classheadrate or config.mc_dropoutrate
        else: 
          self.mc_dropoutboxrate = 0.0
          self.mc_dropoutclassrate = 0.0
        ...
        self.class_net = ClassNet(
            ...
            dropoutrate=self.mc_dropoutclassrate)

        if config.loss_attenuation:
          extend_output = 2*num_anchors # instead of 4*n_anchors => 8*n_anchors
        else:
          extend_output = num_anchors

        self.box_net = BoxNet(
            num_anchors=extend_output,
            num_filters=num_filters,
            min_level=config.min_level,
            max_level=config.max_level,
            is_training_bn=config.is_training_bn,
            ...
            data_format=config.data_format,
            feature_only=feature_only,
            dropoutrate=self.mc_dropoutboxrate)
  ...
  def call(self, inputs, training):
    config = self.config
    if config.mc_dropout and not config.is_training_bn:
      ...
      for _ in range(config.mc_dropoutsamp):
        ...
        if 'object_detection' in config.heads:
          cls = self.class_net(fpn_feats, training)
          box = self.box_net(fpn_feats, training)
      ...
      else: cls_concat = cls # Should be deterministic       
      ...
      else: box_concat = box # Should be deterministic         
      outputs.extend([cls_concat, box_concat])
    else:
      # call backbone network.
      all_feats = self.backbone(inputs, training=training, features_only=True)
      feats = all_feats[config.min_level:config.max_level + 1]
...
class EfficientDetModel(EfficientDetNet):
  ...
  def _postprocess(self, cls_outputs, box_outputs, scales, mode='global'):
    ...
    # takes box outputs and Transforms relative regression coordinates to absolute positions before performing nms
    if not mode:
