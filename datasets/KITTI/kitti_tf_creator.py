...
def generate_val_indices(data_dir, images, split=0.2):
    ...
    validation_set_size = int(
        len(os.listdir(data_dir + "/image_2/")) * split
    )  # Number of images to be used as a validation set.
    validation_set_indexes = random.sample(range(0, len(images)), validation_set_size)
    with open(
        "/app/efficientdet_uncertainty/datasets/KITTI/vaL_index_list.txt", "w"
    ) as file:
        for index in validation_set_indexes:
            file.write(f"{index}\n")


def kitti_csd_tfrecords(
    data_dir,
    output_path,
    classes_to_use,
    label_map_path,
    ...
    annotation_dir = os.path.join(data_dir, "label_2")
    ...
    for img_name in images:
        ...
        if not unlabeled_activate:
            img_anno = read_annotation_file(
                os.path.join(annotation_dir, img_name_without_ext + ".txt")
            )
...
def kitti_active_tfrecords(
    ...
    for img_name in images:
        img_name_without_ext = img_name.split(".")[0]
        anno_file_path = os.path.join(annotation_dir, img_name_without_ext + ".txt")
        if not os.path.exists(anno_file_path):
            continue
        img_anno = read_annotation_file(anno_file_path)
...
def kitti_custom_to_tfrecords(
    ...
    Args:
      data_dir: The full path to the folder containing the data.
      output_path: The path to which TFRecord files will be written. The TFRecord
        with the training set will be located at: <output_path>_train.tfrecord.
      label_map_path: Path to label map proto.
      classes_to_use: List of strings naming the classes for which data should be
        ...
        Adding dontcare class will remove all other bounding boxes that overlap
        with areas marked as dontcare regions.
      get_orig: If True, get original images.
    ...
    train_writer = tf.io.TFRecordWriter("%s_train.tfrecord" % output_path)
    images = sorted([f for f in tf.io.gfile.listdir(data_dir) if f.endswith(".png")])
    dir_orig_act = False
    if len(images) == 0:
        image_names = sorted(
            [f for f in tf.io.gfile.listdir(data_dir) if f.endswith(".txt")]
        )
        image_names = np.asarray([i.split(".")[0] for i in image_names])
        images = sorted(
            [
                f
                for f in tf.io.gfile.listdir(os.path.join(data_dir_orig, "image_2"))
                if f.endswith(".png")
            ]
        )
        images = [i for i in images if i.split(".")[0] in image_names]
        dir_orig_act = True
    img_num = 0
    for img_name in images:
        img_name_without_ext = img_name.split(".")[0]
        img_anno = read_annotation_file(
            os.path.join(data_dir, img_name_without_ext + ".txt")
        )
        if dir_orig_act:
            image_path = os.path.join(os.path.join(data_dir_orig, "image_2"), img_name)
        else:
        ...
        img_num += 1
    if get_orig:
        annotation_dir = os.path.join(data_dir_orig, "label_2")
        image_dir = os.path.join(data_dir_orig, "image_2")
        images = np.asarray(sorted(tf.io.gfile.listdir(image_dir)))[train_indices]
        img_num = 0
        for img_name in images:
            is_validation_img = train_indices[img_num] in validation_indices
            if not is_validation_img:
                img_name_without_ext = img_name.split(".")[0]
                img_anno = read_annotation_file(
                    os.path.join(annotation_dir, img_name_without_ext + ".txt")
                )
                ...
                example = prepare_example(
                    image_path, annotation_for_image, label_map_dict
                )
                train_writer.write(example.SerializeToString())
                train_count += 1
            img_num += 1
...
def convert_kitti_to_tfrecords(
    data_dir, output_path, classes_to_use, label_map_path, validation_indices
):
    """Convert the KITTI detection dataset to TFRecords.
    Args:
      data_dir: The full path to the unzipped folder containing the unzipped data
...
def prepare_example(image_path, annotations, label_map_dict):
    ...
    freatures_dict = {
        ...
        "image/source_id": dataset_util.bytes_feature(
            image_path[-10:-4].lstrip("0").encode("utf8") or "0".encode("utf8")
        ),
    ...
    if "pseudo_score" in annotations.keys():
        freatures_dict["image/object/pseudo_score"] = (
            dataset_util.float_list_feature(annotations["pseudo_score"]),
        )

    example = tf.train.Example(features=tf.train.Features(feature=freatures_dict))
...
def filter_annotations(img_all_annotations, used_classes):
    ...
    """
    dont_care_indices = [
        i for i, x in enumerate(img_all_annotations["type"]) if x not in used_classes
    ]
    ...
    if dont_care_indices:
        ...
        for key in img_all_annotations.keys():
            img_all_annotations[key] = img_all_annotations[key][
...
def read_annotation_file(filename):
    ...
    if "pseudoscore" in filename or "imscore" in filename:
        anno["pseudo_score"] = np.array([float(x[-1]) for x in content])
...
if __name__ == "__main__":
    ...
    label_map_path = (
        "/app/efficientdet_uncertainty/datasets/KITTI/kitti.pbtxt"  # Path to label map
    )
    val_indices = []
    with open(
        "/app/efficientdet_uncertainty/datasets/KITTI/vaL_index_list.txt", "r"
    ) as file:
        for line in file:
