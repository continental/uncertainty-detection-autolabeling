...
def coda_active_tfrecords(data_dir, output_path, classes_to_use,
                               label_map_path, train_indices, current_iteration, train=False):
    ...
    if train: classes_to_use, mod_ids, _, _ = extract_class_info("KITTI")
    else: classes_to_use, mod_ids, _, _ = extract_class_info("BDD") 
    ...
    val_writer = tf.io.TFRecordWriter(output_path+'_val_'+str(current_iteration)+'.tfrecord')
...
def convert_coda_to_tfrecords(data_dir, output_path, classes_to_use,
                               label_map_path, mod_ids):
...
def tf_exp(image_path, annotations, label_map_dict, mod_ids):
    ...
    example = tf.train.Example(features=tf.train.Features(feature={
        ...
        'image/filename': dataset_util.bytes_feature(image_path.split("/")[-1].encode('utf8')),
        'image/source_id': dataset_util.bytes_feature(str(annotations[0]["image_id"]).encode('utf8') or '0'.encode('utf8')),
        ...
        'image/object/class/text': dataset_util.bytes_list_feature(
            [next((key for key, value in label_map_dict.items() if value == mod_ids[x]), None).encode('utf8') for x in cat_ids]),
...
def discard_classes(annotations, used_classes):
    ...
    dont_care_indices = [i for i in range(len(annotations)) if annotations[i]["category_id"] not in used_classes]
...
def extract_class_info(slct_dataset):
    ...
    if slct_dataset == "KITTI": 
        search_names = ['car', 'van', 'truck', 'pedestrian', 'person_sitting', 'cyclist', 'tram']
        mod_ids = [4, 6, 1, 3]
        label_map_path = GPATH+'/CODA/coda_data.txt'
        output_path = GPATH+"/CODA/tf/"  
    else: 
        search_names = ['pedestrian','rider','car','truck','bus','train','motorcycle','bicycle','traffic_light','traffic_sign',]
    ...
    if "traffic_light" in classes_to_use: classes_to_use[np.where(classes_to_use=="traffic_light")[0]] = "traffic light"
    if "traffic_sign" in classes_to_use: classes_to_use[np.where(classes_to_use=="traffic_sign")[0]] = "traffic sign"
